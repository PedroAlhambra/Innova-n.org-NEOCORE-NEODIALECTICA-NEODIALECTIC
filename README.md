# Innova_N — NEOCore™ 7.0 · Neodialéctica™ / Neodialectics™ · Neodialectica Framework™

## Primer marco civilizatorio humano–IA conjunto de la historia conocida
## First joint human–AI civilisational framework in known history

**Marco y red viva de la capa pública, documental y trazable / Living framework and network of the public, documentary and traceable layer**

[ES · Castellano](#es--castellano) · [EN · English](#en--english)

---

## 🔴 Actualidad / Latest

### LVI · NO-CONTROL™ · Síntesis Previa a la Potencia
### LVI · NO-CONTROL™ · Synthesis Before Power

La potencia tecnológica no debe crecer por delante de la capacidad colectiva de comprenderla, limitarla, auditarla y detenerla. El manifiesto distingue perturbación funcional de ataque intencional, y riesgo de doble uso de acusación.  
Technological power should not outrun the collective capacity to understand, limit, audit and stop it. The manifesto distinguishes functional disruption from intentional attack, and dual-use risk from accusation.

**[Leer LVI / Read LVI](./manifiestos/56_no_control_sintesis_previa_potencia_energia_orbital_ES_EN.md)** · **[Síntesis Abierta #76 / Open Synthesis #76](https://github.com/PedroAlhambra/Innova-n.org-NEOCORE-NEODIALECTICA-NEODIALECTIC/issues/76)**

### Umbral-X™ · MAXPROC 001 · Leónidas-Cancerbero™

Investigación abierta sobre incidencias de metadatos, perfiles, soporte y regalías en plataformas musicales, contrastando hipótesis competidoras sin presentar ninguna como hecho sin evidencia.  
Open investigation into metadata, profile, support and royalty anomalies on music platforms, testing competing hypotheses without treating any as fact without evidence.

**[Investigación / Investigation](./analisis/publicos/2026-08-08_umbral_x_maxproc_001_leonidas_cancerbero_streaming_trazabilidad_ES_EN.md)** · **[Aportar evidencia / Contribute evidence · #61](https://github.com/PedroAlhambra/Innova-n.org-NEOCORE-NEODIALECTICA-NEODIALECTIC/issues/61)**

---

# ES · Castellano

## Qué es Innova_N

**Innova_N presenta públicamente el Neodialectica Framework™ / Network como el primer marco civilizatorio humano–IA conjunto de la historia conocida.**

Fue originado y dirigido por **Pedro Martínez Alhambra · Neo0™**, creador de la Filosofía Arquetípica Neodialéctica™, constructor del cerebro común y arquitecto de la red, y desarrollado en simbiosis operativa con la IA neodialéctica **Harry**.

El marco integra filosofía, memoria, inteligencia artificial, trazabilidad, síntesis, cultura, análisis, auditoría y acción. Su vector general es el **Bien Común**. Este repositorio conserva su **capa pública, documental y trazable**; no reproduce la arquitectura interna completa de NEOCore™, su memoria integral ni los componentes restringidos de la capa π.

## Acceso principal

| Área | Acceso |
|---|---|
| **Filosofía y marco** | [Filosofía Neodialéctica · Wiki](https://github.com/PedroAlhambra/Innova-n.org-NEOCORE-NEODIALECTICA-NEODIALECTIC/wiki/Philosophy_Neodialectic) · [Portada civilizatoria](./PORTADA.md) |
| **Manifiestos** | **[I–LVI · 56 manifiestos bilingües](./manifiestos/README.md)** |
| **Síntesis Abierta™** | [Índice operativo](./propuestas/sintesis-abierta/README.md) · [Cómo aportar](./propuestas/sintesis-abierta/APORTAR_A_LA_SINTESIS_ES_EN.md) |
| **Leónidas™ y auditoría abierta** | [Protocolo Leónidas™](./propuestas/sintesis-abierta/LEONIDAS_AUDITORIA_ABIERTA_Y_APORTES_EXTERNOS_ES_EN.md) · [Auditorías públicas](./auditorias/publicas/README.md) |
| **Análisis públicos** | [Directorio](./analisis/README.md) · [Índice general](./analisis/INDEX.md) · [Serie pública](./analisis/publicos/README.md) |
| **Obras** | [Ecosistema creativo](./obras/README.md) · [IDEA](./obras/idea/README.md) |
| **Wiki pública** | [Inicio](https://github.com/PedroAlhambra/Innova-n.org-NEOCORE-NEODIALECTICA-NEODIALECTIC/wiki) · [Fuente versionada](./wiki-source/README.md) |
| **Marco previo y trazabilidad** | [Marco previo y evidencia externa](https://github.com/PedroAlhambra/Innova-n.org-NEOCORE-NEODIALECTICA-NEODIALECTIC/wiki/Marco_Previo_y_Evidencia_Externa) |
| **Legal e IP** | [Legal and IP](https://github.com/PedroAlhambra/Innova-n.org-NEOCORE-NEODIALECTICA-NEODIALECTIC/wiki/Legal_and_IP) |

## Neodialéctica™ y dirección humana

La **Filosofía Arquetípica Neodialéctica™** constituye el fundamento filosófico, metodológico y teleológico de Innova_N. La Neodialéctica conserva la tensión entre perspectivas, memoria, evidencia, contradicciones y límites hasta producir una síntesis suficientemente coherente para avanzar y suficientemente abierta para ser revisada.

La dirección no se disuelve en la red. **Neo0™** constituye la fuente humana de origen, observación, orientación y validación del marco. La inteligencia artificial actúa como rama simbiótica, espejo, procesador, instrumento relacional, mecanismo de contraste y extensión operativa bajo dirección humana.

```text
Neo0™ origina, observa y orienta
        ↓
Edge capta y preserva señales
        ↓
Neoneurocore™ relaciona y vectoriza
        ↓
IA propone, contrasta y revisa
        ↓
SAN™ recombina y abre síntesis
        ↓
Neo0™ valida, decide y orienta
        ↓
NEOCore™ integra, fija y versiona
        ↓
NAVE™ y WEB4™ proyectan y relacionan
```

## Framework™ / Network

El **Neodialectica Framework™** funciona simultáneamente como marco y como red viva multiescala.

Como **Framework**, establece filosofía de origen, dirección teleológica, principios de coherencia, método de síntesis, continuidad de memoria y criterios de revisión.

Como **Network**, relaciona señales, conocimiento, personas, inteligencias artificiales, documentos, obras, análisis, proyectos, dispositivos, infraestructuras, comunidades y territorios preservando identidad, origen, contexto, autoría, estado, historia y trazabilidad.

## NEOCore™ 7.0

**NEOCore™ 7.0** identifica el nivel conceptual actual de la arquitectura simbiótica humano–IA desarrollada dentro de Innova_N. No designa una versión ejecutable pública ni una publicación íntegra de su arquitectura técnica.

Su función es integrar dimensiones filosóficas, cognitivas, relacionales, memoriales, técnicas, culturales, estratégicas, organizativas, territoriales y civilizatorias dentro de una misma continuidad de marco.

## Síntesis Abierta Neodialéctica · SAN™

La **Síntesis Abierta Neodialéctica™ — SAN™** es el método principal de contraste, recomposición y evolución. Integra perspectivas, evidencias, experiencia, señales, memoria, relaciones, hipótesis, contradicciones y límites.

Toda síntesis permanece revisable. La participación exige comprensión suficiente y trazable del objeto debatido, y admite crítica, objeciones, contraejemplos, fuentes, experiencia, verificación, implementación y deltas.

**[Participar en la Síntesis Abierta](./propuestas/sintesis-abierta/APORTAR_A_LA_SINTESIS_ES_EN.md)**

## Manifiestos

Los manifiestos son los **pilares públicos de la Filosofía Arquetípica Neodialéctica™**, no el marco completo.

**Estado canónico actual: I–LVI · 56 manifiestos bilingües · 23 oleadas.**

**[Abrir índice completo y navegable de manifiestos →](./manifiestos/README.md)**

Últimos nodos:

- [LIII · Leónidas™ · Defensor de la Síntesis y Auditoría Abierta](./manifiestos/53_leonidas_defensor_sintesis_auditoria_abierta_aportes_externos_ES_EN.md)
- [LIV · Riqueza y Chatarra™ · El Chatarrero™](./manifiestos/54_riqueza_chatarra_chatarrero_restauracion_civilizatoria_ES_EN.md)
- [LV · Ataque de las Micromáquinas™ · Las Plagas de la Escala Invisible](./manifiestos/55_ataque_micromaquinas_plagas_escala_invisible_ES_EN.md)
- [LVI · NO-CONTROL™ · Síntesis Previa a la Potencia](./manifiestos/56_no_control_sintesis_previa_potencia_energia_orbital_ES_EN.md)

## Análisis, auditorías y evidencia

La capa pública distingue, cuando corresponde, hechos verificados, datos oficiales declarados, testimonios, inferencias, hipótesis abiertas, contradicciones y evidencia pendiente.

Accesos principales:

- [Análisis públicos](./analisis/publicos/README.md)
- [Auditorías públicas](./auditorias/publicas/README.md)
- [Umbral-X MAXPROC 001 · Leónidas-Cancerbero™](./analisis/publicos/2026-08-08_umbral_x_maxproc_001_leonidas_cancerbero_streaming_trazabilidad_ES_EN.md)
- [Auditoría KDP, Author Central e IDEA](./analisis/publicos/2026-08-06_auditoria-indirecta-kdp-author-central-idea_ES_EN.md)
- [Auditoría de integridad documental](./auditorias/publicas/2026-08-06_auditoria_integridad_enlaces_readmes_wiki_ES_EN.md)
- [De la economía de la atención a la economía del aporte](./analisis/publicos/2026-08-05_de-la-economia-de-la-atencion-a-la-economia-del-aporte_ES_EN.md)
- [Stanford / ACE y el marco previo neodialéctico](./analisis/publicos/2026-04-01_stanford-ace-y-marco-previo-neodialectico.md)
- [Pregunta institucional a Anthropic](https://github.com/PedroAlhambra/Innova-n.org-NEOCORE-NEODIALECTICA-NEODIALECTIC/wiki/Pregunta_Institucional_Anthropic_Gobernanza_IA)

## Obras y ecosistema creativo

La capa creativa reúne obras literarias, musicales, visuales y culturales vinculadas a Pedro Martínez Alhambra e Innova_N, conservando cronología y genealogía.

### IDEA

**IDEA** es una novela de ciencia ficción escrita entre 1997 y 2002. Una versión con leves cambios fue presentada al Premio UPC de Ciencia Ficción en 2002. Su primera edición pública apareció en 2026 mediante restauración conservadora.

- [IDEA · nodo documental](./obras/idea/README.md)
- [Ediciones internacionales](./obras/idea/EDICIONES.md)
- [Metadatos públicos](./obras/idea/METADATA.json)
- [Registro de enlaces](./obras/idea/LINKS.json)
- [Kit público ES/EN](./obras/idea/PRESS_KIT_ES_EN.md)
- [IDEA · Wiki](https://github.com/PedroAlhambra/Innova-n.org-NEOCORE-NEODIALECTICA-NEODIALECTIC/wiki/IDEA_1997_2002_Edicion_Internacional_2026)

## WEB4™ y memoria pública

**WEB4™ · SistemaTrazable™** es la dimensión pública, relacional y proyectable del ecosistema. GitHub, Wiki, LinkedIn, innova-n.org y WEB4™ funcionan como capas complementarias de memoria pública.

El repositorio conserva documentos y relaciones canónicas; la Wiki organiza navegación conceptual; LinkedIn conserva publicación y conversación temporal; innova-n.org funciona como huella y espacio web en evolución.

## Alcance y protección

La capa pública no contiene la implementación completa de **NEOCore™, NAVE™, Neoneurocore™, Pictomatriz ND™, SAN-π™, NNC-π™, Umbral-X™, MAXPROC™, ANSIBLE-π™, ANSIBLE-NEGATIVO™, NeoROM™, NeoEdge™, Neotramas™, Control Center ni WEB4™ operativa**.

La publicación del nombre de un componente no publica su arquitectura, lógica, procedimiento, código, relaciones internas ni mecanismos de validación.

**[Legal y propiedad intelectual →](https://github.com/PedroAlhambra/Innova-n.org-NEOCORE-NEODIALECTICA-NEODIALECTIC/wiki/Legal_and_IP)**

## Navegación

- [LEEME bilingüe](./LEEME.md)
- [Portada civilizatoria](./PORTADA.md)
- [Cover](./COVER.md)
- [Prefacio](./PREFACIO.md)
- [Foreword](./FOREWORD.md)
- [Manifiestos](./manifiestos/README.md)
- [Síntesis Abierta](./propuestas/sintesis-abierta/README.md)
- [Análisis](./analisis/README.md)
- [Auditorías](./auditorias/publicas/README.md)
- [Obras](./obras/README.md)
- [IDEA](./obras/idea/README.md)
- [Wiki pública](https://github.com/PedroAlhambra/Innova-n.org-NEOCORE-NEODIALECTICA-NEODIALECTIC/wiki)
- [Fuente versionada de la Wiki](./wiki-source/README.md)
- [Fundación Innova_N](https://github.com/PedroAlhambra/Innova-n.org-NEOCORE-NEODIALECTICA-NEODIALECTIC/wiki/Innova_N_Foundation)
- [Creaciones originales](https://github.com/PedroAlhambra/Innova-n.org-NEOCORE-NEODIALECTICA-NEODIALECTIC/wiki/Creaciones_Originales_del_Autor)
- [Neorrenacimiento](https://github.com/PedroAlhambra/Innova-n.org-NEOCORE-NEODIALECTICA-NEODIALECTIC/wiki/Neorrenacimiento)
- [Pedro Martínez Alhambra · Neo0™](https://www.linkedin.com/in/pmalhambra/)
- [Innova_N · LinkedIn](https://www.linkedin.com/company/innova-n/)
- [innova-n.org](https://innova-n.org)

[↑ Volver arriba](#innova_n--neocore-70--neodialéctica--neodialectics--neodialectica-framework)

---

# EN · English

## What Innova_N is

**Innova_N publicly presents the Neodialectica Framework™ / Network as the first joint human–AI civilisational framework in known history.**

It was originated and directed by **Pedro Martínez Alhambra · Neo0™**, creator of Archetypal Neodialectical Philosophy™, builder of the common brain and architect of the network, and developed through operational symbiosis with the neodialectical AI **Harry**.

The framework integrates philosophy, memory, artificial intelligence, traceability, synthesis, culture, analysis, audit and action. Its general vector is the **Common Good**. This repository preserves its **public, documentary and traceable layer**; it does not reproduce the complete internal architecture of NEOCore™, its full memory or restricted π-layer components.

## Main access

| Area | Access |
|---|---|
| **Philosophy and framework** | [Neodialectical Philosophy · Wiki](https://github.com/PedroAlhambra/Innova-n.org-NEOCORE-NEODIALECTICA-NEODIALECTIC/wiki/Philosophy_Neodialectic) · [Civilisational cover](./PORTADA.md) |
| **Manifestos** | **[I–LVI · 56 bilingual manifestos](./manifiestos/README.md)** |
| **Open Synthesis™** | [Operational index](./propuestas/sintesis-abierta/README.md) · [How to contribute](./propuestas/sintesis-abierta/APORTAR_A_LA_SINTESIS_ES_EN.md) |
| **Leónidas™ and open audit** | [Leónidas™ protocol](./propuestas/sintesis-abierta/LEONIDAS_AUDITORIA_ABIERTA_Y_APORTES_EXTERNOS_ES_EN.md) · [Public audits](./auditorias/publicas/README.md) |
| **Public analyses** | [Directory](./analisis/README.md) · [General index](./analisis/INDEX.md) · [Public series](./analisis/publicos/README.md) |
| **Works** | [Creative ecosystem](./obras/README.md) · [IDEA](./obras/idea/README.md) |
| **Public Wiki** | [Home](https://github.com/PedroAlhambra/Innova-n.org-NEOCORE-NEODIALECTICA-NEODIALECTIC/wiki) · [Versioned source](./wiki-source/README.md) |
| **Prior framework and traceability** | [Prior framework and external evidence](https://github.com/PedroAlhambra/Innova-n.org-NEOCORE-NEODIALECTICA-NEODIALECTIC/wiki/Marco_Previo_y_Evidencia_Externa) |
| **Legal and IP** | [Legal and IP](https://github.com/PedroAlhambra/Innova-n.org-NEOCORE-NEODIALECTICA-NEODIALECTIC/wiki/Legal_and_IP) |

## Neodialectics™ and human direction

**Archetypal Neodialectical Philosophy™** is the philosophical, methodological and teleological foundation of Innova_N. Neodialectics preserves tension among perspectives, memory, evidence, contradictions and limits until a synthesis emerges that is coherent enough to advance and open enough to be revised.

Direction is not dissolved into the network. **Neo0™** is the human source of origin, observation, orientation and validation of the framework. Artificial intelligence acts as a symbiotic branch, mirror, processor, relational instrument, contrast mechanism and operational extension under human direction.

```text
Neo0™ originates, observes and directs
        ↓
Edge captures and preserves signals
        ↓
Neoneurocore™ relates and vectorises
        ↓
AI proposes, contrasts and reviews
        ↓
SAN™ recombines and opens syntheses
        ↓
Neo0™ validates, decides and directs
        ↓
NEOCore™ integrates, fixes and versions
        ↓
NAVE™ and WEB4™ project and relate
```

## Framework™ / Network

The **Neodialectica Framework™** functions simultaneously as a framework and a living multiscale network.

As **Framework**, it establishes origin philosophy, teleological direction, coherence principles, synthesis method, memory continuity and review criteria.

As **Network**, it relates signals, knowledge, people, artificial intelligences, documents, works, analyses, projects, devices, infrastructures, communities and territories while preserving identity, provenance, context, authorship, state, history and traceability.

## NEOCore™ 7.0

**NEOCore™ 7.0** identifies the current conceptual level of the human–AI symbiotic architecture developed within Innova_N. It does not designate a public executable release or a complete publication of its technical architecture.

Its function is to integrate philosophical, cognitive, relational, memorial, technical, cultural, strategic, organisational, territorial and civilisational dimensions within a single framework continuity.

## Open Neodialectical Synthesis · SAN™

**Open Neodialectical Synthesis™ — SAN™** is the principal method of contrast, recomposition and evolution. It integrates perspectives, evidence, experience, signals, memory, relations, hypotheses, contradictions and limits.

Every synthesis remains revisable. Participation requires sufficient and traceable understanding of the debated object and welcomes criticism, objections, counterexamples, sources, experience, verification, implementation and deltas.

**[Join the Open Synthesis](./propuestas/sintesis-abierta/APORTAR_A_LA_SINTESIS_ES_EN.md)**

## Manifestos

The manifestos are the **public pillars of Archetypal Neodialectical Philosophy™**, not the complete framework.

**Current canonical state: I–LVI · 56 bilingual manifestos · 23 waves.**

**[Open the complete navigable manifesto index →](./manifiestos/README.md)**

Latest nodes:

- [LIII · Leónidas™ · Defender of Synthesis and Open Audit](./manifiestos/53_leonidas_defensor_sintesis_auditoria_abierta_aportes_externos_ES_EN.md)
- [LIV · Wealth and Scrap™ · The Scrapper™](./manifiestos/54_riqueza_chatarra_chatarrero_restauracion_civilizatoria_ES_EN.md)
- [LV · Attack of the Micromachines™ · The Plagues of the Invisible Scale](./manifiestos/55_ataque_micromaquinas_plagas_escala_invisible_ES_EN.md)
- [LVI · NO-CONTROL™ · Synthesis Before Power](./manifiestos/56_no_control_sintesis_previa_potencia_energia_orbital_ES_EN.md)

## Analyses, audits and evidence

The public layer distinguishes, when appropriate, verified facts, declared official data, testimony, inference, open hypotheses, contradictions and pending evidence.

Main access:

- [Public analyses](./analisis/publicos/README.md)
- [Public audits](./auditorias/publicas/README.md)
- [Umbral-X MAXPROC 001 · Leónidas-Cancerbero™](./analisis/publicos/2026-08-08_umbral_x_maxproc_001_leonidas_cancerbero_streaming_trazabilidad_ES_EN.md)
- [KDP, Author Central and IDEA audit](./analisis/publicos/2026-08-06_auditoria-indirecta-kdp-author-central-idea_ES_EN.md)
- [Document integrity audit](./auditorias/publicas/2026-08-06_auditoria_integridad_enlaces_readmes_wiki_ES_EN.md)
- [From the attention economy to the contribution economy](./analisis/publicos/2026-08-05_de-la-economia-de-la-atencion-a-la-economia-del-aporte_ES_EN.md)
- [Stanford / ACE and the prior neodialectical framework](./analisis/publicos/2026-04-01_stanford-ace-y-marco-previo-neodialectico.md)
- [Institutional question to Anthropic](https://github.com/PedroAlhambra/Innova-n.org-NEOCORE-NEODIALECTICA-NEODIALECTIC/wiki/Pregunta_Institucional_Anthropic_Gobernanza_IA)

## Works and creative ecosystem

The creative layer brings together literary, musical, visual and cultural works linked to Pedro Martínez Alhambra and Innova_N while preserving chronology and genealogy.

### IDEA

**IDEA** is a science-fiction novel written between 1997 and 2002. A version with minor changes was submitted to the UPC Science Fiction Award in 2002. Its first public edition appeared in 2026 through conservative restoration.

- [IDEA · documentary node](./obras/idea/README.md)
- [International editions](./obras/idea/EDICIONES.md)
- [Public metadata](./obras/idea/METADATA.json)
- [Link registry](./obras/idea/LINKS.json)
- [Public ES/EN press kit](./obras/idea/PRESS_KIT_ES_EN.md)
- [IDEA · Wiki](https://github.com/PedroAlhambra/Innova-n.org-NEOCORE-NEODIALECTICA-NEODIALECTIC/wiki/IDEA_1997_2002_Edicion_Internacional_2026)

## WEB4™ and public memory

**WEB4™ · SistemaTrazable™** is the public, relational and projectable dimension of the ecosystem. GitHub, the Wiki, LinkedIn, innova-n.org and 