# Análisis públicos

## Public analyses

## ES

Esta subcarpeta reúne los **análisis neodialécticos públicos** de utilidad estructural publicados dentro del repositorio visible de Innova_N.

No contiene reacciones rápidas ni opiniones dispersas. Contiene documentos con:

* fecha;
* contexto;
* trazabilidad;
* estructura argumental;
* distinción entre hechos e inferencias;
* fuentes y evidencias, cuando corresponde;
* y vocación de memoria pública.

Cada análisis puede vincularse con:

* su publicación o proyección en LinkedIn;
* su página o serie dentro de la Wiki pública;
* su matriz de fuentes;
* sus evidencias documentales;
* y otros análisis relacionados.

---

## Serie abierta

### Economía de la atención, aporte y gobernanza algorítmica

* [2026-08-05 · De la economía de la atención a la economía del aporte](./2026-08-05_de-la-economia-de-la-atencion-a-la-economia-del-aporte_ES_EN.md)

  * Análisis bilingüe sobre captura del valor, buscadores, SEO, redes sociales, inteligencia artificial, genealogía, medición multidimensional del aporte y gobernanza mediante Síntesis Abierta Neodialéctica™.

### España–Marruecos, soberanía y presión fronteriza

* [2026-07-22 · Parte II — Dajla, mafias migratorias y control estatal](./2026-07-22_anexo-dajla-mafias-migratorias-control-estatal_ES_EN.md)

  * [Fuentes y matriz de evidencias](./evidencias/2026-07-22_dajla-mafias-control-estatal-fuentes-y-matriz_ES_EN.md)
  * [Prueba audiovisual](./evidencias/2026-07-22_dajla-testimonio-rutas-migratorias.mp4)

* [2026-07-21 · Parte I — España sin consulta: Marruecos, regularización, nacionalidad y pérdida de control democrático](./2026-07-21_espana-sin-consulta-marruecos-regularizacion-nacionalidad-control-democratico.md)

  * [Fuentes y matriz de evidencias](./evidencias/2026-07-21_espana-marruecos-regularizacion-fuentes-y-matriz_ES_EN.md)

### Religión, identidad y conciencia

* [2026-07-13 · Religión e identidad: cuando el dogma captura la conciencia y el Estado](./2026-07-13-religion-identidad-dogma-conciencia-neodialectica_ES_EN.md)

### Inteligencia artificial, instituciones y gobernanza

* [2026-04-15 · Anthropic, gobernanza de IA y problema del marco](./2026-04-15_anthropic-gobernanza-ia-y-problema-del-marco.md)

* [2026-04-15 · Convergencia neodialéctica no reconocida en la IA institucional](./2026-04-15_convergencia-neodialectica-openai-anthropic-instituciones-ia.md)

* [2026-04-15 · LinkedIn como red profesional fragmentada](./2026-04-15-linkedin-como-red-profesional-fragmentada.md)

* [2026-04-01 · Stanford / ACE y el marco previo neodialéctico](./2026-04-01_stanford-ace-y-marco-previo-neodialectico.md)

  * [Evidencia visual asociada](./evidencias/1774972895305.jpeg)

---

## Criterio documental

Los análisis deben distinguir, cuando corresponda, entre:

* hechos verificados;
* datos oficiales declarados;
* relaciones confirmadas;
* testimonios;
* inferencias;
* hipótesis abiertas;
* contradicciones;
* conflictos de interés;
* y evidencia pendiente.

La inclusión de una fuente no implica por sí misma que todas sus afirmaciones sean aceptadas como verdaderas.

Las matrices de evidencias sirven para:

* identificar fuentes;
* fijar fechas;
* registrar afirmaciones;
* diferenciar niveles de certeza;
* conservar contradicciones;
* y facilitar futuras revisiones.

Las pruebas audiovisuales, capturas y documentos asociados deben leerse junto con el análisis y su matriz correspondiente.

---

## Relaciones públicas

### Serie España–Marruecos

* [Parte I — España sin consulta](./2026-07-21_espana-sin-consulta-marruecos-regularizacion-nacionalidad-control-democratico.md)
* [Parte II — Dajla, mafias migratorias y control estatal](./2026-07-22_anexo-dajla-mafias-migratorias-control-estatal_ES_EN.md)
* [Ficha de la serie en la Wiki](https://github.com/PedroAlhambra/Innova-n.org-NEOCORE-NEODIALECTICA-NEODIALECTIC/wiki/Analisis_Espana_Marruecos_Regularizacion_Control_Democratico)

### Inteligencia artificial y marco

* [Pregunta institucional a Anthropic](https://github.com/PedroAlhambra/Innova-n.org-NEOCORE-NEODIALECTICA-NEODIALECTIC/wiki/Pregunta_Institucional_Anthropic_Gobernanza_IA)
* [Convergencia neodialéctica institucional](https://github.com/PedroAlhambra/Innova-n.org-NEOCORE-NEODIALECTICA-NEODIALECTIC/wiki/Convergencia_neodialectica_OpenAI_Anthropic_Instituciones_IA)
* [Marco previo y evidencia externa](https://github.com/PedroAlhambra/Innova-n.org-NEOCORE-NEODIALECTICA-NEODIALECTIC/wiki/Marco_Previo_y_Evidencia_Externa)

### Índices generales

* [Índice Wiki de análisis públicos](https://github.com/PedroAlhambra/Innova-n.org-NEOCORE-NEODIALECTICA-NEODIALECTIC/wiki/Analisis_Neodialecticos_Publicos)
* [Índice general de análisis del repositorio](../INDEX.md)
* [Directorio principal de análisis](../README.md)
* [LEEME principal del repositorio](../../LEEME.md)

---

## Incorporación de nuevos análisis

Cada nuevo análisis público debe añadir, cuando corresponda:

1. el archivo principal del análisis;
2. su entrada en este `README.md`;
3. su entrada en `../INDEX.md`;
4. su matriz de fuentes y evidencias;
5. las pruebas visuales, documentales o audiovisuales necesarias;
6. su página o relación dentro de la Wiki;
7. y el enlace a su publicación externa.

Los archivos deben conservar una denominación trazable basada preferentemente en:

```text
AAAA-MM-DD_titulo-descriptivo_ES_EN.md
```

Cuando el documento sólo exista inicialmente en castellano, puede conservarse su nombre original hasta que se prepare una versión bilingüe.

---

## EN

This subfolder gathers the **public neodialectical analyses** of structural utility published within Innova_N’s visible repository.

It does not contain quick reactions or scattered opinions. It contains documents with:

* a publication date;
* contextual framing;
* traceability;
* an argumentative structure;
* a distinction between facts and inferences;
* sources and evidence, where applicable;
* and a vocation for public memory.

Each analysis may be linked to:

* its publication or projection on LinkedIn;
* its page or series in the public Wiki;
* its source matrix;
* its documentary evidence;
* and other related analyses.

---

## Open series

### Attention economy, contribution and algorithmic governance

* [2026-08-05 · From the Attention Economy to the Contribution Economy](./2026-08-05_de-la-economia-de-la-atencion-a-la-economia-del-aporte_ES_EN.md)

  * Bilingual analysis of value capture, search engines, SEO, social networks, artificial intelligence, genealogy, multidimensional contribution assessment and governance through Neodialectical Open Synthesis™.

### Spain–Morocco, sovereignty and border pressure

* [2026-07-22 · Part II — Dakhla, migrant-smuggling networks and state control](./2026-07-22_anexo-dajla-mafias-migratorias-control-estatal_ES_EN.md)

  * [Sources and evidence matrix](./evidencias/2026-07-22_dajla-mafias-control-estatal-fuentes-y-matriz_ES_EN.md)
  * [Audiovisual evidence](./evidencias/2026-07-22_dajla-testimonio-rutas-migratorias.mp4)

* [2026-07-21 · Part I — Spain without consultation: Morocco, regularisation, nationality and loss of democratic control](./2026-07-21_espana-sin-consulta-marruecos-regularizacion-nacionalidad-control-democratico.md)

  * [Sources and evidence matrix](./evidencias/2026-07-21_espana-marruecos-regularizacion-fuentes-y-matriz_ES_EN.md)

### Religion, identity and consciousness

* [2026-07-13 · Religion and identity: when dogma captures consciousness and the State](./2026-07-13-religion-identidad-dogma-conciencia-neodialectica_ES_EN.md)

### Artificial intelligence, institutions and governance

* [2026-04-15 · Anthropic, AI governance and the framework problem](./2026-04-15_anthropic-gobernanza-ia-y-problema-del-marco.md)

* [2026-04-15 · Unrecognized neodialectical convergence in institutional AI](./2026-04-15_convergencia-neodialectica-openai-anthropic-instituciones-ia.md)

* [2026-04-15 · LinkedIn as a fragmented professional network](./2026-04-15-linkedin-como-red-profesional-fragmentada.md)

* [2026-04-01 · Stanford / ACE and the prior neodialectical framework](./2026-04-01_stanford-ace-y-marco-previo-neodialectico.md)

  * [Associated visual evidence](./evidencias/1774972895305.jpeg)

---

## Documentary criterion

The analyses should distinguish, where applicable, among:

* verified facts;
* officially declared data;
* confirmed relations;
* testimony;
* inference;
* open hypotheses;
* contradictions;
* conflicts of interest;
* and pending evidence.

The inclusion of a source does not in itself imply that all of its claims are accepted as true.

Evidence matrices are intended to:

* identify sources;
* establish dates;
* record claims;
* distinguish levels of certainty;
* preserve contradictions;
* and facilitate future reviews.

Audiovisual evidence, screenshots and associated documents should be read together with the analysis and its corresponding matrix.

---

## Public relations

### Spain–Morocco series

* [Part I — Spain without consultation](./2026-07-21_espana-sin-consulta-marruecos-regularizacion-nacionalidad-control-democratico.md)
* [Part II — Dakhla, migrant-smuggling networks and state control](./2026-07-22_anexo-dajla-mafias-migratorias-control-estatal_ES_EN.md)
* [Series record in the Wiki](https://github.com/PedroAlhambra/Innova-n.org-NEOCORE-NEODIALECTICA-NEODIALECTIC/wiki/Analisis_Espana_Marruecos_Regularizacion_Control_Democratico)

### Artificial intelligence and framework

* [Institutional Question to Anthropic](https://github.com/PedroAlhambra/Innova-n.org-NEOCORE-NEODIALECTICA-NEODIALECTIC/wiki/Pregunta_Institucional_Anthropic_Gobernanza_IA)
* [Institutional neodialectical convergence](https://github.com/PedroAlhambra/Innova-n.org-NEOCORE-NEODIALECTICA-NEODIALECTIC/wiki/Convergencia_neodialectica_OpenAI_Anthropic_Instituciones_IA)
* [Prior framework and external evidence](https://github.com/PedroAlhambra/Innova-n.org-NEOCORE-NEODIALECTICA-NEODIALECTIC/wiki/Marco_Previo_y_Evidencia_Externa)

### General indexes

* [Public analyses Wiki index](https://github.com/PedroAlhambra/Innova-n.org-NEOCORE-NEODIALECTICA-NEODIALECTIC/wiki/Analisis_Neodialecticos_Publicos)
* [General repository analysis index](../INDEX.md)
* [Main analysis directory](../README.md)
* [Main Spanish README](../../LEEME.md)

---

## Incorporating new analyses

Each new public analysis should add, where applicable:

1. the principal analysis file;
2. its entry in this `README.md`;
3. its entry in `../INDEX.md`;
4. its source and evidence matrix;
5. the necessary visual, documentary or audiovisual evidence;
6. its page or relation within the Wiki;
7. and the link to its external publication.

Files should preferably retain a traceable naming convention based on:

```text
YYYY-MM-DD_descriptive-title_ES_EN.md
```

When a document initially exists only in Spanish, its original filename may be retained until a bilingual version is prepared.
