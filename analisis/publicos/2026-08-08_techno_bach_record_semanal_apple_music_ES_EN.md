# Techno Bach · máximo semanal observado en Apple Music for Artists
# Techno Bach · highest observed weekly Apple Music for Artists total

**Fecha / Date:** 2026-08-08  
**Proyecto musical / Music project:** Techno Bach  
**Fuente / Source:** correos semanales de Apple Music for Artists conservados en Gmail  
**Estado / Status:** registro descriptivo · no extrapolación comercial / descriptive record · no commercial extrapolation

---

# ES · Castellano

## 1. Dato nuevo

El resumen semanal de Apple Music for Artists correspondiente al periodo **31 de julio – 6 de agosto de 2026** informa de:

- **20 reproducciones**;
- **1 Shazam**.

## 2. Comparación con los resúmenes semanales disponibles

La búsqueda de todos los correos de resumen semanal de Apple Music for Artists conservados actualmente en Gmail para **Techno Bach** devuelve la siguiente serie única por periodo:

| Periodo | Reproducciones | Shazam |
|---|---:|---:|
| 3–9 julio | 3 | 0 |
| 10–16 julio | 15 | 2 |
| 17–23 julio | 19 | 1 |
| 24–30 julio | 14 | 0 |
| 31 julio–6 agosto | **20** | 1 |

El periodo 24–30 de julio aparece varias veces en Gmail con el mismo valor; se contabiliza una sola vez para no duplicar el dato semanal.

## 3. Dictamen

**20 reproducciones es el máximo semanal observado en el historial de resúmenes semanales actualmente disponible en Gmail para Techno Bach**, superando el máximo anterior de 19 reproducciones del 17–23 de julio.

Esto permite llamarlo **récord semanal observado** dentro de esta serie documental.

No permite afirmar todavía que sea el máximo histórico absoluto del proyecto si existieran periodos anteriores no conservados o no enviados por correo.

## 4. Lectura proporcional

La progresión disponible muestra que Techno Bach pasó de 3 reproducciones semanales a un máximo observado de 20 en aproximadamente un mes, aunque con oscilaciones intermedias.

El dato es pequeño en escala absoluta y no debe exagerarse, pero sí constituye una señal positiva verificable de actividad orgánica en Apple Music sin que este registro permita atribuir causalidad a una campaña concreta.

## 5. Relación con la auditoría de regalías

Este dato pertenece a **Apple Music**, no a Spotify, y por tanto **no prueba** el problema de regalías Spotify–DistroKid ni permite trasladar cifras entre plataformas.

Sí aporta una referencia independiente de que el catálogo continúa registrando actividad de escucha real en otra plataforma.

- [Actualización DistroKid ticket 4499471 · 2026-08-08](./2026-08-08_distrokid_ticket_4499471_respuesta_no_resolutiva_y_reiteracion_auditoria_ES_EN.md)
- [Spotify–DistroKid · cierre circular y escalado](./2026-08-07_spotify_distrokid_cierre_circular_y_escalado_ES_EN.md)

---

# EN · English

## 1. New data point

The Apple Music for Artists weekly summary corresponding to the period **31 July – 6 August 2026** reports:

- **20 plays**;
- **1 Shazam**.

## 2. Comparison with the available weekly summaries

The search across all Apple Music for Artists weekly-summary emails currently preserved in Gmail for **Techno Bach** returns the following unique series by period:

| Period | Plays | Shazam |
|---|---:|---:|
| 3–9 July | 3 | 0 |
| 10–16 July | 15 | 2 |
| 17–23 July | 19 | 1 |
| 24–30 July | 14 | 0 |
| 31 July–6 August | **20** | 1 |

The 24–30 July period appears several times in Gmail with the same value; it is counted only once so that the weekly figure is not duplicated.

## 3. Finding

**20 plays is the highest weekly total observed in the history of weekly summaries currently available in Gmail for Techno Bach**, exceeding the previous high of 19 plays from 17–23 July.

This allows it to be described as an **observed weekly record** within this documentary series.

It does not yet allow us to state that it is the project's absolute all-time high if earlier periods exist that were not preserved or were not sent by email.

## 4. Proportional reading

The available progression shows that Techno Bach went from 3 weekly plays to an observed high of 20 in approximately one month, although with intermediate fluctuations.

The figure is small in absolute scale and should not be exaggerated, but it does constitute a positive, verifiable signal of organic activity on Apple Music, without this record allowing causality to be attributed to any specific campaign.

## 5. Relation to the royalty audit

This data belongs to **Apple Music**, not Spotify, and therefore **does not prove** the Spotify–DistroKid royalty problem or allow figures to be transferred between platforms.

It does provide an independent reference showing that the catalogue continues to register real listening activity on another platform.

- [DistroKid ticket 4499471 update · 2026-08-08](./2026-08-08_distrokid_ticket_4499471_respuesta_no_resolutiva_y_reiteracion_auditoria_ES_EN.md)
- [Spotify–DistroKid · circular closure and escalation](./2026-08-07_spotify_distrokid_cierre_circular_y_escalado_ES_EN.md)

---

**Pedro Martínez Alhambra · Innova_N**