# Análisis

## Analysis

## ES

Esta carpeta reúne los materiales públicos de análisis, auditoría y trazabilidad documental del repositorio visible de **Innova_N**.

No debe leerse como un archivo indiferenciado. Funciona como una carpeta madre con dos líneas principales:

* **auditorías**, cuando el objeto principal es revisar, contrastar, verificar o documentar un caso concreto;
* **análisis públicos**, cuando el objeto principal es producir una lectura estructural de utilidad pública desde el marco neodialéctico.

Los documentos pueden relacionarse con matrices de fuentes, evidencias visuales o audiovisuales, publicaciones externas y páginas de la Wiki pública.

## Subseries

* [Índice general de análisis](./INDEX.md)
* [Análisis públicos](./publicos/README.md)
* [Directorio de análisis públicos](./publicos/)
* [Directorio de auditorías](./auditorias/)

## Análisis públicos

### Serie España–Marruecos

* [2026-07-22 · Parte II — Dajla, mafias migratorias y control estatal](./publicos/2026-07-22_anexo-dajla-mafias-migratorias-control-estatal_ES_EN.md)

  * [Fuentes y matriz de evidencias](./publicos/evidencias/2026-07-22_dajla-mafias-control-estatal-fuentes-y-matriz_ES_EN.md)
  * [Prueba audiovisual](./publicos/evidencias/2026-07-22_dajla-testimonio-rutas-migratorias.mp4)

* [2026-07-21 · Parte I — España sin consulta: Marruecos, regularización, nacionalidad y pérdida de control democrático](./publicos/2026-07-21_espana-sin-consulta-marruecos-regularizacion-nacionalidad-control-democratico.md)

  * [Fuentes y matriz de evidencias](./publicos/evidencias/2026-07-21_espana-marruecos-regularizacion-fuentes-y-matriz_ES_EN.md)

### Religión, identidad y conciencia

* [2026-07-13 · Religión e identidad: cuando el dogma captura la conciencia y el Estado](./publicos/2026-07-13-religion-identidad-dogma-conciencia-neodialectica_ES_EN.md)

### Inteligencia artificial, instituciones y gobernanza

* [2026-04-15 · Anthropic, gobernanza de IA y problema del marco](./publicos/2026-04-15_anthropic-gobernanza-ia-y-problema-del-marco.md)

* [2026-04-15 · Convergencia neodialéctica no reconocida en la IA institucional](./publicos/2026-04-15_convergencia-neodialectica-openai-anthropic-instituciones-ia.md)

* [2026-04-15 · LinkedIn como red profesional fragmentada](./publicos/2026-04-15-linkedin-como-red-profesional-fragmentada.md)

* [2026-04-01 · Stanford / ACE y el marco previo neodialéctico](./publicos/2026-04-01_stanford-ace-y-marco-previo-neodialectico.md)

  * [Evidencia visual asociada](./publicos/evidencias/1774972895305.jpeg)

## Enlaces relacionados

* [Serie España–Marruecos: ficha de la Wiki](https://github.com/PedroAlhambra/Innova-n.org-NEOCORE-NEODIALECTICA-NEODIALECTIC/wiki/Analisis_Espana_Marruecos_Regularizacion_Control_Democratico)

* [Análisis Neodialécticos Públicos](https://github.com/PedroAlhambra/Innova-n.org-NEOCORE-NEODIALECTICA-NEODIALECTIC/wiki/Analisis_Neodialecticos_Publicos)

* [Pregunta institucional a Anthropic sobre gobernanza de IA](https://github.com/PedroAlhambra/Innova-n.org-NEOCORE-NEODIALECTICA-NEODIALECTIC/wiki/Pregunta_Institucional_Anthropic_Gobernanza_IA)

* [Convergencia neodialéctica en la IA institucional: OpenAI, Anthropic y el problema del marco](https://github.com/PedroAlhambra/Innova-n.org-NEOCORE-NEODIALECTICA-NEODIALECTIC/wiki/Convergencia_neodialectica_OpenAI_Anthropic_Instituciones_IA)

* [Marco previo y evidencia externa](https://github.com/PedroAlhambra/Innova-n.org-NEOCORE-NEODIALECTICA-NEODIALECTIC/wiki/Marco_Previo_y_Evidencia_Externa)

* [Serie completa de análisis públicos](./publicos/README.md)

* [Índice completo de análisis](./INDEX.md)

## Criterio documental

Los análisis públicos deben distinguir, cuando corresponda, entre:

* hechos verificados;
* datos oficiales declarados;
* relaciones confirmadas;
* testimonios;
* inferencias;
* hipótesis abiertas;
* contradicciones;
* conflictos de interés;
* y evidencia pendiente.

Las matrices y evidencias asociadas no sustituyen al análisis principal. Funcionan como soporte documental, registro de fuentes y mecanismo de trazabilidad.

## Estado actual

Se conserva en esta carpeta, sin mover por ahora, el archivo:

* [2025-12 · Evento-Reflejo · Auditoría DistroKid](./2025-12_Evento-Reflejo_Auditoria-Distrokid.md)

Su permanencia en esta ruta preserva la continuidad documental y evita romper enlaces anteriores.

La reorganización conceptual se aplica desde ahora sin alterar todavía esa traza pública.

---

## EN

This folder gathers the public analysis, audit and documentary traceability materials of the visible **Innova_N** repository.

It should not be read as an undifferentiated archive. It operates as a parent folder with two principal lines:

* **audits**, when the main purpose is to review, contrast, verify or document a specific case;
* **public analyses**, when the main purpose is to produce a structural public-utility reading from the neodialectical framework.

The documents may be connected to source matrices, visual or audiovisual evidence, external publications and pages in the public Wiki.

## Subseries

* [General analysis index](./INDEX.md)
* [Public analyses](./publicos/README.md)
* [Public analysis directory](./publicos/)
* [Audit directory](./auditorias/)

## Public analyses

### Spain–Morocco series

* [2026-07-22 · Part II — Dakhla, migrant-smuggling networks and state control](./publicos/2026-07-22_anexo-dajla-mafias-migratorias-control-estatal_ES_EN.md)

  * [Sources and evidence matrix](./publicos/evidencias/2026-07-22_dajla-mafias-control-estatal-fuentes-y-matriz_ES_EN.md)
  * [Audiovisual evidence](./publicos/evidencias/2026-07-22_dajla-testimonio-rutas-migratorias.mp4)

* [2026-07-21 · Part I — Spain without consultation: Morocco, regularisation, nationality and loss of democratic control](./publicos/2026-07-21_espana-sin-consulta-marruecos-regularizacion-nacionalidad-control-democratico.md)

  * [Sources and evidence matrix](./publicos/evidencias/2026-07-21_espana-marruecos-regularizacion-fuentes-y-matriz_ES_EN.md)

### Religion, identity and consciousness

* [2026-07-13 · Religion and identity: when dogma captures consciousness and the State](./publicos/2026-07-13-religion-identidad-dogma-conciencia-neodialectica_ES_EN.md)

### Artificial intelligence, institutions and governance

* [2026-04-15 · Anthropic, AI governance and the framework problem](./publicos/2026-04-15_anthropic-gobernanza-ia-y-problema-del-marco.md)

* [2026-04-15 · Unrecognized neodialectical convergence in institutional AI](./publicos/2026-04-15_convergencia-neodialectica-openai-anthropic-instituciones-ia.md)

* [2026-04-15 · LinkedIn as a fragmented professional network](./publicos/2026-04-15-linkedin-como-red-profesional-fragmentada.md)

* [2026-04-01 · Stanford / ACE and the prior neodialectical framework](./publicos/2026-04-01_stanford-ace-y-marco-previo-neodialectico.md)

  * [Associated visual evidence](./publicos/evidencias/1774972895305.jpeg)

## Related links

* [Spain–Morocco series: Wiki record](https://github.com/PedroAlhambra/Innova-n.org-NEOCORE-NEODIALECTICA-NEODIALECTIC/wiki/Analisis_Espana_Marruecos_Regularizacion_Control_Democratico)

* [Public Neodialectical Analyses](https://github.com/PedroAlhambra/Innova-n.org-NEOCORE-NEODIALECTICA-NEODIALECTIC/wiki/Analisis_Neodialecticos_Publicos)

* [Institutional Question to Anthropic about AI governance](https://github.com/PedroAlhambra/Innova-n.org-NEOCORE-NEODIALECTICA-NEODIALECTIC/wiki/Pregunta_Institucional_Anthropic_Gobernanza_IA)

* [Neodialectical convergence in institutional AI: OpenAI, Anthropic and the framework problem](https://github.com/PedroAlhambra/Innova-n.org-NEOCORE-NEODIALECTICA-NEODIALECTIC/wiki/Convergencia_neodialectica_OpenAI_Anthropic_Instituciones_IA)

* [Prior framework and external evidence](https://github.com/PedroAlhambra/Innova-n.org-NEOCORE-NEODIALECTICA-NEODIALECTIC/wiki/Marco_Previo_y_Evidencia_Externa)

* [Complete public analysis series](./publicos/README.md)

* [Complete analysis index](./INDEX.md)

## Documentary criterion

Public analyses should distinguish, where applicable, among:

* verified facts;
* officially declared data;
* confirmed relations;
* testimony;
* inference;
* open hypotheses;
* contradictions;
* conflicts of interest;
* and pending evidence.

Associated matrices and evidence do not replace the principal analysis. They operate as documentary support, source records and traceability mechanisms.

## Current state

The following file remains in this folder without being moved for now:

* [2025-12 · Event-Reflection · DistroKid Audit](./2025-12_Evento-Reflejo_Auditoria-Distrokid.md)

Its permanence in this path preserves documentary continuity and avoids breaking previous links.

The conceptual reorganization applies from this point onward without yet altering that public trace.
