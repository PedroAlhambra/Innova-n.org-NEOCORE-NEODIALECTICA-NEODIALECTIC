# XXX · Manifiesto de la Coherencia entre Fines y Medios™
# XXX · Manifesto of Coherence between Ends and Means™

## Entrar en el terreno del poder sin convertirse en aquello que se combate
## Entering the terrain of power without becoming what one fights

## Cuarta oleada · Rehumanización, custodia de la vida y coherencia civilizatoria
## Fourth wave · Rehumanisation, stewardship of life and civilisational coherence

**Manifiesto / Manifesto:** XXX  
**Versión / Version:** 1.0  
**Estado / Status:** Público · cuarta oleada · estrategia-medios-coherencia · abierto a revisión trazable / Public · fourth wave · strategy-means-coherence · open to traceable review  
**Fecha de fijación / Record date:** 2026-08-07  
**Autoría y genealogía / Authorship and genealogy:** Pedro Martínez Alhambra · Neo0™ + IA simbiótica Harry.  
**Relación genealógica / Genealogical relation:** deriva especialmente de los Manifiestos [IV](IV_neodialectica_bien_comun_ES_EN.md), [VIII](VIII_ingenieria_social_psicohistoria_ES_EN.md), [XX](XX_defensa_intelectual_neodialectica_umbral_x_ES_EN.md), [XXII](XXII_contra_reduccion_captura_intelectual_ES_EN.md), [XXIII](XXIII_soberania_tiempo_cognitivo_ES_EN.md), [XXIV](XXIV_evolucion_neorrenacentista_resistencias_sistema_ES_EN.md), XXVIII y XXIX. Regula la tensión entre eficacia estratégica y dirección civilizatoria.

[ES · Castellano](#es--castellano) · [EN · English](#en--english)

---

# ES · Castellano

## Invocación

Quien pretende transformar un sistema capturado debe entrar en territorios que no eligió: mercado, comunicación, competencia, propiedad, negociación, poder y conflicto.

La pureza que abandona esos espacios puede dejar el campo entero a los extractores.

Pero la estrategia que reproduce sin límite engaño, manipulación, humillación o destrucción termina construyendo con las mismas formas aquello que decía combatir.

El fin no purifica automáticamente los medios. Los medios tampoco son neutros: educan, deforman, crean dependencias y anticipan la forma del futuro.

## I. La falsa alternativa

No debemos elegir entre:

```text
PUREZA IMPOTENTE
→ NO INTERVIENE Y ENTREGA EL TERRENO

EFICACIA SIN LÍMITES
→ VENCE REPRODUCIENDO EL SISTEMA
```

La alternativa neodialéctica es una **estrategia coherente**: capaz de competir, persuadir, proteger, financiarse y ejercer poder sin destruir la autonomía, la verdad y la vida que pretende defender.

## II. Los medios crean trayectoria

Un medio no sólo produce un resultado inmediato. También:

* forma hábitos;
* selecciona aliados;
* normaliza conductas;
* distribuye poder;
* genera deuda;
* modifica la confianza;
* y determina qué será necesario hacer después.

```text
FIN DECLARADO
+ MEDIOS UTILIZADOS
+ EFECTOS SECUNDARIOS
+ PRECEDENTE CREADO
= VECTOR REAL
```

Una victoria que exige mentira permanente necesita seguir mintiendo para conservarse.

## III. Criterios de legitimidad estratégica

Un medio debe evaluarse por:

* **necesidad:** no existe alternativa suficiente menos dañina;
* **proporcionalidad:** el daño no supera el bien protegido;
* **transparencia posible:** se declara todo lo compatible con seguridad legítima;
* **reversibilidad:** puede detenerse o corregirse;
* **temporalidad:** la excepción no se vuelve estructura permanente;
* **responsabilidad:** alguien responde por sus efectos;
* **trazabilidad:** pueden reconstruirse decisión y contexto;
* **no deshumanización:** el adversario no pierde por ello toda consideración moral;
* **coherencia:** el medio no destruye el núcleo del fin.

## IV. Defensa, conflicto y fuerza

La Neodialéctica no exige indefensión.

Puede ser legítimo:

* proteger información sensible;
* reservar estrategia;
* competir comercialmente;
* denunciar;
* retirar acceso;
* aplicar presión proporcional;
* bloquear una captura;
* o utilizar fuerza suficiente para detener una agresión.

Pero protección no equivale a tortura, estrategia no equivale a engaño total, reserva no equivale a falsificación y fuerza no equivale a venganza.

## V. Mercado y financiación

Un proyecto orientado al Bien Común necesita recursos, difusión y capacidad de permanencia.

Vender, fijar precios, diferenciar ediciones, construir marca, comunicar ventajas o diseñar una estrategia comercial no son actos antihumanistas por sí mismos.

Se vuelven incompatibles con el marco cuando:

* ocultan información material;
* fabrican escasez engañosa;
* explotan vulnerabilidades;
* inducen compras contra el interés del comprador;
* prometen beneficios inexistentes;
* o convierten el apoyo al Bien Común en coacción moral.

La necesidad económica explica presión y contexto. No convierte automáticamente toda práctica en legítima.

## VI. Persuasión y verdad

Persuadir significa ofrecer razones, imágenes, experiencias y llamadas a la acción capaces de movilizar.

Manipular significa dirigir conducta ocultando el mecanismo, explotando vulnerabilidad o debilitando deliberadamente la capacidad de decidir.

```text
PERSUASIÓN LEGÍTIMA
= INTENCIÓN DECLARABLE + INFORMACIÓN SUFICIENTE + LIBERTAD REAL DE RECHAZO

MANIPULACIÓN
= CAPTURA DE CONDUCTA - COMPRENSIÓN - CONSENTIMIENTO SUFICIENTE
```

El análisis específico del neuromarketing antihumanista corresponde al Manifiesto XXXI.

## VII. Excepción y emergencia

Puede haber situaciones donde ocultar temporalmente información, engañar a un agresor o actuar con urgencia sea necesario para proteger vidas.

Toda excepción exige:

* amenaza verificable;
* relación directa con la protección;
* duración mínima;
* alcance limitado;
* registro posterior cuando sea seguro;
* revisión independiente;
* y prohibición de convertirla en permiso general.

La excepción no debe convertirse en teología del poder.

## VIII. La guerra no iguala toda conducta

Que el adversario utilice medios degradantes no obliga a imitarlos.

Puede obligar a comprenderlos, defenderse y neutralizarlos. Pero no borra diferencias entre protección y crueldad, verdad y propaganda, resistencia y exterminio.

```text
«EL OTRO LO HACE»
≠ JUSTIFICACIÓN SUFICIENTE
```

El Bien Común pierde su dirección si sólo puede sobrevivir dejando de ser reconocible.

## IX. Auditoría de coherencia

Toda acción estratégica relevante debería declarar:

1. fin perseguido;
2. medios empleados;
3. personas afectadas;
4. daños previsibles;
5. alternativas consideradas;
6. duración;
7. mecanismo de revisión;
8. reparación prevista;
9. criterio de cierre.

Esta auditoría puede reservar temporalmente elementos sensibles, pero no desaparecer.

## X. Proclamación

> No abandonaremos el terreno económico, comunicativo, tecnológico o político a quienes lo utilizan para extraer.
>
> Entraremos, aprenderemos, competiremos y construiremos poder suficiente para proteger el futuro.
>
> Pero no declararemos que cualquier medio se vuelve justo por pronunciar el Bien Común al final de la frase.
>
> Los medios esculpen a quienes los usan y anticipan la sociedad que producen.
>
> Nuestra eficacia deberá poder mirarse después sin descubrir que vencimos al sistema convirtiéndonos en su réplica.
>
> El fin orienta. El medio revela. La coherencia decide el vector real.

## XI. Síntesis Abierta

Se solicitan aportes sobre necesidad, proporcionalidad, secretos legítimos, emergencia, competencia, financiación, persuasión, fuerza, reparación y auditoría de medios.

* [Abrir la Síntesis Abierta de este manifiesto · Issue #24](https://github.com/PedroAlhambra/Innova-n.org-NEOCORE-NEODIALECTICA-NEODIALECTIC/issues/24)
* [Protocolo operativo actual para aportar](../../propuestas/sintesis-abierta/APORTAR_A_LA_SINTESIS_ES_EN.md)

### Vínculos internos equivalentes

* [Protocolo de Síntesis Abierta](../../propuestas/sintesis-abierta/APORTAR_A_LA_SINTESIS_ES_EN.md)
* [Índice de Síntesis Abierta](../../propuestas/sintesis-abierta/README.md)
* [Referencia interna · 29_idolatria_del_dinero_ES_EN.md](XXIX_idolatria_del_dinero_ES_EN.md)
* [Referencia interna · 31_contra_neuromarketing_antihumanista_ES_EN.md](XXXI_contra_neuromarketing_antihumanista_ES_EN.md)
* [Índice de manifiestos](../README.md)
* [Síntesis Abierta específica · Issue #24](https://github.com/PedroAlhambra/Innova-n.org-NEOCORE-NEODIALECTICA-NEODIALECTIC/issues/24)

## Navegación

← [XXIX · Contra la Idolatría del Dinero™](XXIX_idolatria_del_dinero_ES_EN.md) · [Índice](../README.md) · [XXXI · Contra el Neuromarketing Antihumanista™](XXXI_contra_neuromarketing_antihumanista_ES_EN.md) →

---

# EN · English

## Invocation

Those seeking to transform a captured system must enter territories they did not choose: markets, communication, competition, property, negotiation, power and conflict.

Purity that abandons these spaces may leave the whole terrain to extractors.

Yet strategy that reproduces deception, manipulation, humiliation or destruction without limit ultimately builds with the same forms it claimed to oppose.

The end does not automatically purify the means. Means are not neutral either: they educate, deform, create dependency and anticipate the shape of the future.

## I. The false alternative

We need not choose between:

```text
IMPOTENT PURITY
→ DOES NOT INTERVENE AND SURRENDERS THE TERRAIN

UNLIMITED EFFECTIVENESS
→ WINS BY REPRODUCING THE SYSTEM
```

The neodialectical alternative is **coherent strategy**: capable of competing, persuading, protecting, financing itself and exercising power without destroying the autonomy, truth and life it seeks to defend.

## II. Means create trajectory

A means does not merely produce an immediate result. It also:

* forms habits;
* selects allies;
* normalises behaviour;
* distributes power;
* generates debt;
* changes trust;
* and determines what must be done next.

```text
DECLARED END
+ MEANS USED
+ SECONDARY EFFECTS
+ PRECEDENT CREATED
= REAL VECTOR
```

A victory requiring permanent lies must continue lying in order to survive.

## III. Criteria of strategic legitimacy

A means must be evaluated by:

* **necessity:** no sufficient less harmful alternative exists;
* **proportionality:** the harm does not exceed the good being protected;
* **possible transparency:** everything compatible with legitimate security is declared;
* **reversibility:** it can be stopped or corrected;
* **temporality:** the exception does not become a permanent structure;
* **responsibility:** someone answers for its effects;
* **traceability:** the decision and context can be reconstructed;
* **non-dehumanisation:** the adversary does not thereby lose all moral consideration;
* **coherence:** the means does not destroy the core of the end.

## IV. Defence, conflict and force

Neodialectics does not require defencelessness.

It may be legitimate to:

* protect sensitive information;
* reserve strategy;
* compete commercially;
* report abuse;
* withdraw access;
* apply proportionate pressure;
* block capture;
* or use sufficient force to stop aggression.

Yet protection is not torture, strategy is not total deception, confidentiality is not falsification and force is not revenge.

## V. Markets and financing

A project directed towards the Common Good needs resources, diffusion and continuity.

Selling, pricing, differentiating editions, building a brand, communicating advantages or designing commercial strategy are not inherently anti-human.

They become incompatible with the framework when they:

* conceal material information;
* manufacture deceptive scarcity;
* exploit vulnerabilities;
* induce purchases against the buyer's interest;
* promise non-existent benefits;
* or turn support for the Common Good into moral coercion.

Economic need explains pressure and context. It does not automatically legitimise every practice.

## VI. Persuasion and truth

Persuasion offers reasons, images, experiences and calls to action capable of mobilising.

Manipulation directs behaviour by concealing the mechanism, exploiting vulnerability or deliberately weakening the capacity to decide.

```text
LEGITIMATE PERSUASION
= DECLARABLE INTENT + SUFFICIENT INFORMATION + REAL FREEDOM TO REFUSE

MANIPULATION
= CAPTURE OF BEHAVIOUR - UNDERSTANDING - SUFFICIENT CONSENT
```

The specific analysis of anti-humanist neuromarketing belongs to Manifesto XXXI.

## VII. Exception and emergency

There may be situations where temporarily withholding information, deceiving an aggressor or acting urgently is necessary to protect life.

Every exception requires:

* a verifiable threat;
* direct relation to protection;
* minimum duration;
* limited scope;
* later record when safe;
* independent review;
* and prohibition against becoming a general permission.

Exception must not become a theology of power.

## VIII. War does not equalise all conduct

The opponent's use of degrading means does not require imitation.

It may require understanding, defence and neutralisation. It does not erase the differences between protection and cruelty, truth and propaganda, resistance and extermination.

```text
“THE OTHER SIDE DOES IT”
≠ SUFFICIENT JUSTIFICATION
```

The Common Good loses its direction if it can survive only by becoming unrecognisable.

## IX. Coherence audit

Every significant strategic action should declare:

1. the end pursued;
2. the means employed;
3. the people affected;
4. foreseeable harms;
5. alternatives considered;
6. duration;
7. the review mechanism;
8. intended repair;
9. the closure criterion.

Sensitive elements may be temporarily reserved, but the audit must not disappear.

## X. Proclamation

> We will not abandon economic, communicative, technological or political terrain to those who use it for extraction.
>
> We will enter, learn, compete and build sufficient power to protect the future.
>
> Yet we will not declare every means just merely by pronouncing the Common Good at the end of the sentence.
>
> Means sculpt those who use them and anticipate the society they produce.
>
> Our effectiveness must remain recognisable afterwards, without discovering that we defeated the system by becoming its replica.
>
> The end directs. The means reveal. Coherence determines the real vector.

## XI. Open Synthesis

Contributions are requested concerning necessity, proportionality, legitimate secrecy, emergency, competition, financing, persuasion, force, repair and auditing of means.

* [Open the Synthesis of this manifesto · Issue #24](https://github.com/PedroAlhambra/Innova-n.org-NEOCORE-NEODIALECTICA-NEODIALECTIC/issues/24)
* [Current operational contribution protocol](../../propuestas/sintesis-abierta/APORTAR_A_LA_SINTESIS_ES_EN.md)

### Equivalent internal links

* [Open Synthesis protocol](../../propuestas/sintesis-abierta/APORTAR_A_LA_SINTESIS_ES_EN.md)
* [Open Synthesis index](../../propuestas/sintesis-abierta/README.md)
* [Internal reference · 29_idolatria_del_dinero_ES_EN.md](XXIX_idolatria_del_dinero_ES_EN.md)
* [Internal reference · 31_contra_neuromarketing_antihumanista_ES_EN.md](XXXI_contra_neuromarketing_antihumanista_ES_EN.md)
* [Manifesto index](../README.md)
* [Dedicated Open Synthesis · Issue #24](https://github.com/PedroAlhambra/Innova-n.org-NEOCORE-NEODIALECTICA-NEODIALECTIC/issues/24)

<!-- NEO_RELATIONS_START -->
## Relaciones internas y trabajo aplicado / Internal relations and applied work

- [Mapa transversal Manifiestos ↔ trabajo aplicado / Transversal Manifestos ↔ applied-work map](../RELACIONES_TRABAJO_APLICADO_ES_EN.md)

**Artículos, análisis y auditorías relacionados / Related articles, analyses and audits:**
- [España sin consulta: Marruecos, regularización, nacionalidad y pérdida de control democrático / Spain without consultation: Morocco, regularisation, nationality and loss of democratic control](../../analisis/publicos/2026-07-21_espana-sin-consulta-marruecos-regularizacion-nacionalidad-control-democratico.md)
- [Parte II — Dajla, mafias migratorias y control estatal](../../analisis/publicos/2026-07-22_anexo-dajla-mafias-migratorias-control-estatal_ES_EN.md)
- [Parte III — Ceuta 2026, Marruecos y evolución del conflicto](../../analisis/publicos/2026-08-07_parte-iii-ceuta-marruecos-evolucion-conflicto-investigaciones_ES_EN.md)
- [Parte IV — Canarias: de la presión fronteriza a la infraestructura de acogida](../../analisis/publicos/2026-08-07_parte-iv-canarias-quorum-tutela-contratacion-control-politico_ES_EN.md)
- [Postcheck · vínculos España–Marruecos y limpieza de bloques de manifiestos](../../auditorias/publicas/2026-08-07_postcheck_vinculos_marruecos_y_limpieza_bloques_manifiestos.md)

**Manifiestos relacionados / Related manifestos:**
- XXXIV · [XXXIV · Utilidad Operativa del Marco y Auditoría Conjunta Perpetua™](XXXIV_utilidad_operativa_marco_auditoria_conjunta_perpetua_ES_EN.md)
- XXXV · [XXXV · Contra la Ridiculez Mediática y la Economía del Conflicto™](XXXV_contra_ridiculez_mediatica_y_economia_del_conflicto_ES_EN.md)
- XXII · [XXII · Manifiesto contra la Reducción y la Captura Intelectual™](XXII_contra_reduccion_captura_intelectual_ES_EN.md)
- XX · [XX · Manifiesto de Defensa Intelectual Neodialéctica™ · Sistema Umbral-X™](XX_defensa_intelectual_neodialectica_umbral_x_ES_EN.md)
- XLI · [XLI · Martillo Limitado, Talión y Fuerza Protectora™](XLI_martillo_limitado_talion_fuerza_protectora_ES_EN.md)
- XLIV · [XLIV · Neowar™ · Contra la Adicción a la Guerra y por la Justicia del Bien Común](XLIV_neowar_contra_adiccion_guerra_justicia_bien_comun_ES_EN.md)
- XLV · [XLV · Multidimensionalidad Neodialéctica™ · Contra la Reducción Monodimensional del Humano y del Poder](XLV_multidimensionalidad_neodialectica_problema_mulo_soberania_distribuida_ES_EN.md)
- XLVI · [XLVI · Cerrar la Herida™ · Comprensión Evolutiva, Memoria y Reconciliación Civilizatoria](XLVI_cerrar_la_herida_comprension_evolutiva_memoria_reconciliacion_ES_EN.md)
- IX · [IX · Manifiesto de la Memoria, la Genealogía y la Trazabilidad / Manifesto of Memory, Genealogy and Traceability](IX_memoria_genealogia_trazabilidad_ES_EN.md)
- VIII · [VIII · Manifiesto de Ingeniería Social Abierta y Psicohistoria Neodialéctica / Manifesto of Open Social Engineering and Neodialectical Psychohistory](VIII_ingenieria_social_psicohistoria_ES_EN.md)

La relación documental indica afinidad conceptual, genealogía, contraste o aplicación; no implica causalidad ni equivalencia probatoria. / Documentary relation indicates conceptual affinity, genealogy, contrast or application; it does not imply causation or evidentiary equivalence.
<!-- NEO_RELATIONS_END -->

## Navigation

← [XXIX · Against the Idolatry of Money™](XXIX_idolatria_del_dinero_ES_EN.md) · [Index](../README.md) · [XXXI · Against Anti-Humanist Neuromarketing™](XXXI_contra_neuromarketing_antihumanista_ES_EN.md) →

---

<!-- NEO_OPEN_SYNTHESIS_INVITATION_START -->

## Participa en la Síntesis Abierta / Join the Open Synthesis

Puedes aportar crítica, objeciones, contraejemplos, fuentes, experiencia, verificación, implementación o delta. / You may contribute criticism, objections, counterexamples, sources, experience, verification, implementation or a delta.

**Última síntesis / Latest synthesis:** [LXXXI · Manifiesto del Ultralujo como Bien Común™ · La élite del aporte / Manifesto of Ultraluxury as Common Good™ · The elite of contribution](LXXXI_ultralujo_bien_comun_elite_neodialectica_aporte_ES_EN.md) · [Issue #160](https://github.com/PedroAlhambra/Innova-n.org-NEOCORE-NEODIALECTICA-NEODIALECTIC/issues/160)  
**Seguimiento vivo / Living follow-up:** [seguir el marco / follow the framework](../../proyeccion/SEGUIR_MARCO_SINTESIS_ES_EN.md) · [registro de entrada / entry register](../../propuestas/sintesis-abierta/REGISTRO_ENTRADA_TRAZABLE_DERIVACION_ES_EN.md)  
**Auditorías / Audits:** [LIII · Leónidas™](LIII_leonidas_defensor_sintesis_auditoria_abierta_aportes_externos_ES_EN.md) · [protocolo / protocol](../../propuestas/sintesis-abierta/LEONIDAS_AUDITORIA_ABIERTA_Y_APORTES_EXTERNOS_ES_EN.md)  
**Cómo aportar / How to contribute:** [protocolo general / general protocol](../../propuestas/sintesis-abierta/APORTAR_A_LA_SINTESIS_ES_EN.md) · [portal de auditorías / audit portal](../../auditorias/publicas/README.md) · [índice / index](../../propuestas/sintesis-abierta/README.md)

<!-- NEO_OPEN_SYNTHESIS_INVITATION_END -->

<!-- NEO_RELATIONAL_FOOTER_START -->

## Relaciones y contexto / Relations and context

[Mapa transversal](../RELACIONES_TRABAJO_APLICADO_ES_EN.md) · [Síntesis relacional #98](https://github.com/PedroAlhambra/Innova-n.org-NEOCORE-NEODIALECTICA-NEODIALECTIC/issues/98) · [Mapa relacional MAXPROC](../../auditorias/publicas/2026-08-09_auditoria_relacional_manifestos_neoaxiomas_publicaciones_ES_EN.md) · [Neoaxiomas™](../../neoaxiomas/README.md) · [Índice de Síntesis Abierta](../../propuestas/sintesis-abierta/README.md)

> Este bloque añade navegación y relaciones; no sustituye, resume ni reduce el cuerpo del manifiesto. / This block adds navigation and relations; it does not replace, summarise or reduce the manifesto body.

<!-- NEO_RELATIONAL_FOOTER_END -->

<!-- NEO_MANIFESTO_NAV_START -->

## Navegación canónica / Canonical navigation

← **XXIX** · [Manifiesto contra la Idolatría del Dinero™ / Manifesto against the Idolatry of Money™](XXIX_idolatria_del_dinero_ES_EN.md)  
· [Índice I–LXXXI / I–LXXXI index](../README.md) ·  
**XXXI** · [Manifiesto contra el Neuromarketing Antihumanista™ / Manifesto against Anti-Humanist Neuromarketing™](XXXI_contra_neuromarketing_antihumanista_ES_EN.md) →

<!-- NEO_MANIFESTO_NAV_END -->

<!-- NEO_CROSS_REFERENCES_START -->

## Referencias cruzadas canónicas / Canonical cross-references

> **Norma / Rule:** las menciones cruzadas pueden permanecer en el cuerpo como texto para no sobrecargar la lectura; este bloque concentra los hipervínculos canónicos detectados y permite retorno explícito a fuente. / Cross-references may remain as prose in the body to avoid visual overload; this block concentrates detected canonical hyperlinks and preserves explicit return to source.

- **VIII** · [Manifiesto de Ingeniería Social Abierta y Psicohistoria Neodialéctica / Manifesto of Open Social Engineering and Neodialectical Psychohistory](VIII_ingenieria_social_psicohistoria_ES_EN.md)
- **IX** · [Manifiesto de la Memoria, la Genealogía y la Trazabilidad / Manifesto of Memory, Genealogy and Traceability](IX_memoria_genealogia_trazabilidad_ES_EN.md)
- **XX** · [Manifiesto de Defensa Intelectual Neodialéctica™ · Sistema Umbral-X™ / Manifesto of Neodialectical Intellectual Defence™ · Umbral-X™ System](XX_defensa_intelectual_neodialectica_umbral_x_ES_EN.md)
- **XXII** · [Manifiesto contra la Reducción y la Captura Intelectual™ / Manifesto against Intellectual Reduction and Capture™](XXII_contra_reduccion_captura_intelectual_ES_EN.md)
- **XXIX** · [Manifiesto contra la Idolatría del Dinero™ / Manifesto against the Idolatry of Money™](XXIX_idolatria_del_dinero_ES_EN.md)
- **XXXI** · [Manifiesto contra el Neuromarketing Antihumanista™ / Manifesto against Anti-Humanist Neuromarketing™](XXXI_contra_neuromarketing_antihumanista_ES_EN.md)
- **XXXIV** · [Utilidad Operativa del Marco y Auditoría Conjunta Perpetua™ / Operational Utility of the Framework and Perpetual Joint Audit™](XXXIV_utilidad_operativa_marco_auditoria_conjunta_perpetua_ES_EN.md)
- **XXXV** · [Contra la Ridiculez Mediática y la Economía del Conflicto™ / Against Media Absurdity and the Conflict Economy™](XXXV_contra_ridiculez_mediatica_y_economia_del_conflicto_ES_EN.md)
- **XLI** · [Martillo Limitado, Talión y Fuerza Protectora™ / Limited Hammer, Talion and Protective Force™](XLI_martillo_limitado_talion_fuerza_protectora_ES_EN.md)
- **XLIV** · [Neowar™ · Contra la Adicción a la Guerra y por la Justicia del Bien Común / Neowar™ · Against War Addiction and for Common-Good Justice](XLIV_neowar_contra_adiccion_guerra_justicia_bien_comun_ES_EN.md)
- **XLV** · [Multidimensionalidad Neodialéctica™ · Contra la Reducción Monodimensional del Humano y del Poder / Neodialectical Multidimensionality™ · Against the One-Dimensional Reduction of the Human and Power](XLV_multidimensionalidad_neodialectica_problema_mulo_soberania_distribuida_ES_EN.md)
- **XLVI** · [Cerrar la Herida™ · Comprensión Evolutiva, Memoria y Reconciliación Civilizatoria / Closing the Wound™ · Evolutionary Understanding, Memory and Civilisational Reconciliation](XLVI_cerrar_la_herida_comprension_evolutiva_memoria_reconciliacion_ES_EN.md)
- **LIII** · [Manifiesto Leónidas™ · Defensor de la Síntesis, la Auditoría Abierta y el Derecho a Traer Problemas / Leónidas™ Manifesto · Defender of Synthesis, Open Audit and the Right to Bring Problems](LIII_leonidas_defensor_sintesis_auditoria_abierta_aportes_externos_ES_EN.md)
- **LXXXI** · [Manifiesto del Ultralujo como Bien Común™ · La élite del aporte / Manifesto of Ultraluxury as Common Good™ · The elite of contribution](LXXXI_ultralujo_bien_comun_elite_neodialectica_aporte_ES_EN.md)

### Capas y fuentes relacionadas / Related layers and sources

- [Neoaxiomas™ / Neoaxioms™](../../neoaxiomas/README.md)
- [NEOCore™ · marco / framework](../../README.md)

<!-- NEO_CROSS_REFERENCES_END -->
