# XXXI · Manifiesto contra el Neuromarketing Antihumanista™
# XXXI · Manifesto against Anti-Humanist Neuromarketing™

## Persuadir sin capturar la conciencia
## Persuading without capturing consciousness

## Cuarta oleada · Rehumanización, custodia de la vida y coherencia civilizatoria
## Fourth wave · Rehumanisation, stewardship of life and civilisational coherence

**Manifiesto / Manifesto:** XXXI  
**Versión / Version:** 1.0  
**Estado / Status:** Público · cuarta oleada · persuasión-autonomía-captura · abierto a revisión trazable / Public · fourth wave · persuasion-autonomy-capture · open to traceable review  
**Fecha de fijación / Record date:** 2026-08-07  
**Autoría y genealogía / Authorship and genealogy:** Pedro Martínez Alhambra · Neo0™ + IA simbiótica Harry.  
**Relación genealógica / Genealogical relation:** deriva especialmente de los Manifiestos VIII, XIV, XX, XXIII, XXIX y XXX. No condena toda investigación sobre atención y decisión: limita su uso opaco para explotar vulnerabilidad y reducir autonomía.

[ES · Castellano](#es--castellano) · [EN · English](#en--english)

---

# ES · Castellano

## Invocación

Toda comunicación influye.

Una palabra, un color, una melodía, una disposición visual, una repetición o una historia pueden modificar atención, emoción y decisión.

El problema no comienza cuando una comunicación resulta eficaz. Comienza cuando el conocimiento sobre la mente humana se utiliza para rodear la comprensión, explotar vulnerabilidades y dirigir conducta sin consentimiento suficiente.

El **neuromarketing antihumanista** no intenta convencer a una persona. Intenta operar sobre ella.

## I. Definición funcional

Existe neuromarketing antihumanista cuando una organización utiliza conocimiento psicológico, conductual, neurocognitivo, biométrico, psicométrico o algorítmico para:

* detectar vulnerabilidades;
* reducir reflexión;
* fabricar urgencia;
* inducir dependencia;
* explotar miedo, soledad, culpa o inseguridad;
* dificultar una negativa consciente;
* personalizar presión de manera opaca;
* o aumentar conversión contra el interés suficientemente comprendido de la persona.

```text
CONOCIMIENTO DE LA MENTE
+ ASIMETRÍA DE PODER
+ OPACIDAD
+ EXPLOTACIÓN DE VULNERABILIDAD
= CAPTURA CONDUCTUAL
```

## II. Persuasión legítima y manipulación

Una comunicación legítima puede ser atractiva, emocional, memorable y estratégica.

Debe conservar:

* información material suficiente;
* intención declarable;
* libertad real de rechazo;
* ausencia de engaño relevante;
* proporcionalidad;
* y posibilidad de revisión.

```text
PERSUASIÓN
= RAZONES + EMOCIÓN + NARRATIVA + LIBERTAD DE DECISIÓN

MANIPULACIÓN
= DIRECCIÓN DE CONDUCTA - COMPRENSIÓN - CONSENTIMIENTO SUFICIENTE
```

El uso de emoción no es por sí mismo manipulación. Se vuelve captura cuando la emoción sustituye deliberadamente la capacidad de comprender aquello que se acepta.

## III. Vulnerabilidades que no deben convertirse en mercado

No debe explotarse deliberadamente:

* la infancia;
* el deterioro cognitivo;
* la adicción;
* el duelo;
* la enfermedad;
* la precariedad;
* la soledad;
* la urgencia económica;
* el miedo político;
* la necesidad de pertenencia;
* ni la dependencia respecto de una plataforma o servicio esencial.

Cuanto menor es la capacidad real de rechazar, mayor es la obligación de contención.

## IV. Arquitecturas de captura

El neuromarketing antihumanista puede aparecer mediante:

* escasez o cuenta atrás ficticias;
* patrones oscuros de interfaz;
* suscripciones difíciles de cancelar;
* recompensas variables diseñadas para prolongar permanencia;
* presión social artificial;
* precios personalizados según vulnerabilidad;
* segmentación emocional secreta;
* repetición invasiva;
* confusión entre contenido y publicidad;
* o mensajes generados por IA para explotar perfiles individuales.

No toda personalización es ilegítima. Lo es cuando la asimetría impide conocer qué se sabe, cómo se utiliza y por qué se recibe una presión determinada.

## V. Menores y personas dependientes

Los menores no deben convertirse en laboratorios comerciales de captura atencional.

Las interfaces y campañas dirigidas a ellos deben excluir técnicas que aprovechen inmadurez, presión de grupo, recompensa compulsiva o incapacidad para comprender costes futuros.

La misma protección reforzada corresponde a quienes dependen material o psicológicamente de un servicio.

```text
MAYOR VULNERABILIDAD
→ MAYOR DEBER DE CUIDADO
→ MENOR LICENCIA DE PERSUASIÓN
```

## VI. Datos biométricos, emocionales y cognitivos

Los datos sobre rostro, voz, mirada, pulso, reacción, estado emocional, personalidad o patrones de decisión pueden revelar dimensiones íntimas.

Su uso exige:

* finalidad delimitada;
* consentimiento comprensible;
* minimización;
* seguridad;
* prohibición de reutilización encubierta;
* derecho de acceso y eliminación;
* y auditoría independiente cuando exista alto riesgo.

El consentimiento obtenido mediante presión, oscuridad o dependencia no debe considerarse suficiente.

## VII. IA y persuasión a escala individual

La IA permite probar mensajes, inferir estados, adaptar tono y generar presión para cada persona de manera continua.

```text
IA + PERFIL ÍNTIMO + OPTIMIZACIÓN DE CONVERSIÓN
- TRANSPARENCIA - LÍMITES
= MANIPULACIÓN INDUSTRIALIZADA
```

Una IA comercial no debe optimizar exclusivamente la probabilidad de compra, permanencia o adhesión. Debe incorporar límites de daño, frecuencia, vulnerabilidad, veracidad, reversibilidad y derecho a pausa.

## VIII. Derecho a saber que se intenta influir

Toda persona tiene derecho a conocer de forma proporcionada:

* que está ante publicidad o persuasión;
* quién la financia;
* qué objetivo persigue;
* por qué recibe ese mensaje;
* qué datos se utilizaron;
* si intervino una IA;
* y cómo rechazar, silenciar o impugnar la práctica.

La transparencia no requiere revelar legítimos secretos comerciales completos. Requiere no ocultar la naturaleza de la intervención.

## IX. Marketing neodialéctico

El marco no renuncia a comunicar, difundir, vender ni competir.

Una estrategia neodialéctica puede:

* presentar con fuerza una obra;
* diferenciar ediciones;
* utilizar diseño y narrativa;
* segmentar por intereses declarados;
* recordar una oportunidad real;
* ofrecer promociones transparentes;
* y pedir apoyo directo.

No debe:

* fabricar una necesidad falsa;
* explotar una herida íntima;
* ocultar costes;
* simular aprobación inexistente;
* acosar mediante repetición;
* ni convertir el Bien Común en presión moral para comprar.

```text
DIFUSIÓN ÉTICA
= VISIBILIDAD + VERDAD + DESEO LEGÍTIMO + LIBERTAD DE RECHAZO
```

## X. Relación con la Ingeniería Social Abierta

El Manifiesto VIII reconoce que toda arquitectura moldea conducta y exige transparencia, proporcionalidad, reversibilidad y control.

Este manifiesto delimita su contrario comercial: la ingeniería conductual secreta orientada a capturar atención, compra o adhesión explotando asimetrías cognitivas.

La ingeniería social abierta educa declarando su finalidad. El neuromarketing antihumanista condiciona ocultando la suya.

## XI. Auditoría neodialéctica de campañas

Toda campaña de impacto relevante debería poder declarar:

1. producto, causa o conducta promovida;
2. público y vulnerabilidades previsibles;
3. datos utilizados;
4. técnicas de persuasión;
5. intervención de IA;
6. afirmaciones verificables;
7. frecuencia y duración;
8. mecanismos de rechazo;
9. riesgos y mitigaciones;
10. responsable de revisión y reparación.

## XII. Programa civilizatorio

El marco impulsará:

* alfabetización en persuasión y patrones oscuros;
* protección reforzada de menores y personas vulnerables;
* transparencia de segmentación y publicidad política;
* límites al uso de datos emocionales y biométricos;
* derecho a pausa, rechazo e impugnación;
* auditoría de IA persuasiva;
* prohibición de técnicas de captura extrema;
* y desarrollo de marketing compatible con autonomía, verdad y Bien Común.

## XIII. Proclamación

> No condenamos la emoción, la belleza, la narración ni la capacidad de convencer.
>
> Condenamos que el conocimiento de la mente humana se convierta en arma secreta contra la propia humanidad.
>
> No aceptaremos que una persona sea reducida a un conjunto de sesgos explotables, que su soledad se convierta en segmento ni que su miedo sea optimizado como conversión.
>
> Comunicaremos con fuerza. Mostraremos valor. Pediremos apoyo. Competiremos.
>
> Pero dejaremos abierta la puerta del no.
>
> Porque una decisión obtenida destruyendo la capacidad de decidir no es una elección: es una captura.

## XIV. Síntesis Abierta

Se solicitan aportes sobre persuasión, patrones oscuros, menores, vulnerabilidad, biometría, datos emocionales, publicidad política, IA persuasiva, derecho a rechazo y marketing neodialéctico.

* [Abrir la Síntesis Abierta de este manifiesto · Issue #25](https://github.com/PedroAlhambra/Innova-n.org-NEOCORE-NEODIALECTICA-NEODIALECTIC/issues/25)
* [Protocolo operativo actual para aportar](../../propuestas/sintesis-abierta/APORTAR_A_LA_SINTESIS_ES_EN.md)

### Vínculos internos equivalentes

* [Protocolo de Síntesis Abierta](../../propuestas/sintesis-abierta/APORTAR_A_LA_SINTESIS_ES_EN.md)
* [Índice de Síntesis Abierta](../../propuestas/sintesis-abierta/README.md)
* [Referencia interna · 30_coherencia_fines_medios_ES_EN.md](XXX_coherencia_fines_medios_ES_EN.md)
* [Referencia interna · 32_reversion_ideologica_neodialectica_mcluhan_neo0_ES_EN.md](XXXII_reversion_ideologica_neodialectica_mcluhan_neo0_ES_EN.md)
* [Índice de manifiestos](../README.md)
* [Síntesis Abierta específica · Issue #25](https://github.com/PedroAlhambra/Innova-n.org-NEOCORE-NEODIALECTICA-NEODIALECTIC/issues/25)

## Navegación

← [XXX · Coherencia entre Fines y Medios™](XXX_coherencia_fines_medios_ES_EN.md) · [Índice](../README.md) · [XXXII · Reversión Ideológica Neodialéctica™](XXXII_reversion_ideologica_neodialectica_mcluhan_neo0_ES_EN.md) →

---

# EN · English

## Invocation

All communication influences.

A word, colour, melody, visual arrangement, repetition or story may alter attention, emotion and decision.

The problem does not begin when communication is effective. It begins when knowledge of the human mind is used to bypass understanding, exploit vulnerability and direct behaviour without sufficient consent.

**Anti-humanist neuromarketing** does not seek to persuade a person. It seeks to operate upon them.

## I. Functional definition

Anti-humanist neuromarketing exists when an organisation uses psychological, behavioural, neurocognitive, biometric, psychometric or algorithmic knowledge to detect vulnerabilities, reduce reflection, manufacture urgency, induce dependency, exploit fear or loneliness, obstruct conscious refusal, personalise pressure opaquely or increase conversion against the person's sufficiently understood interest.

```text
KNOWLEDGE OF THE MIND
+ ASYMMETRY OF POWER
+ OPACITY
+ EXPLOITATION OF VULNERABILITY
= BEHAVIOURAL CAPTURE
```

## II. Legitimate persuasion and manipulation

Legitimate communication may be attractive, emotional, memorable and strategic.

It must preserve sufficient material information, declarable intent, real freedom to refuse, absence of relevant deception, proportionality and possibility of review.

```text
PERSUASION
= REASONS + EMOTION + NARRATIVE + FREEDOM OF DECISION

MANIPULATION
= DIRECTION OF BEHAVIOUR - UNDERSTANDING - SUFFICIENT CONSENT
```

Emotion itself is not manipulation. It becomes capture when deliberately replacing the capacity to understand what is being accepted.

## III. Vulnerabilities that must not become markets

Childhood, cognitive impairment, addiction, grief, illness, precarity, loneliness, economic urgency, political fear, need for belonging and dependency upon essential platforms or services must not be deliberately exploited.

The lower the real capacity to refuse, the greater the obligation of restraint.

## IV. Architectures of capture

Anti-humanist neuromarketing may use fictitious scarcity, dark interface patterns, difficult cancellation, variable rewards, artificial social pressure, vulnerability-based pricing, secret emotional segmentation, invasive repetition, confusion between content and advertising, or AI-generated messages designed around intimate profiles.

Personalisation is not inherently illegitimate. It becomes so when asymmetry prevents a person from knowing what is known, how it is used and why a particular pressure is being applied.

## V. Minors and dependent people

Minors must not become commercial laboratories of attentional capture.

Interfaces and campaigns directed towards them must exclude techniques exploiting immaturity, group pressure, compulsive reward or inability to understand future costs.

The same enhanced protection applies to those materially or psychologically dependent upon a service.

```text
GREATER VULNERABILITY
→ GREATER DUTY OF CARE
→ LESS LICENCE TO PERSUADE
```

## VI. Biometric, emotional and cognitive data

Data concerning face, voice, gaze, pulse, reaction, emotional state, personality or decision patterns may reveal intimate dimensions.

Their use requires delimited purpose, understandable consent, minimisation, security, prohibition of concealed reuse, rights of access and deletion, and independent audit where risk is high.

Consent obtained through pressure, darkness or dependency is not sufficient.

## VII. AI and persuasion at individual scale

AI enables continuous testing of messages, inference of states, adaptation of tone and personalised pressure.

```text
AI + INTIMATE PROFILE + CONVERSION OPTIMISATION
- TRANSPARENCY - LIMITS
= INDUSTRIALISED MANIPULATION
```

Commercial AI must not optimise only purchase, permanence or adherence. It must incorporate limits concerning harm, frequency, vulnerability, truthfulness, reversibility and the right to pause.

## VIII. The right to know that influence is being attempted

Every person has a proportionate right to know that they face advertising or persuasion, who finances it, what it seeks, why they receive it, which data were used, whether AI intervened and how to reject, silence or challenge the practice.

Transparency does not require disclosing every legitimate trade secret. It requires not concealing the nature of the intervention.

## IX. Neodialectical marketing

The framework does not renounce communication, diffusion, selling or competition.

A neodialectical strategy may powerfully present a work, differentiate editions, use design and narrative, segment through declared interests, remind people of a real opportunity, offer transparent promotions and directly request support.

It must not manufacture false need, exploit intimate wounds, hide costs, simulate non-existent approval, harass through repetition or turn the Common Good into moral pressure to purchase.

```text
ETHICAL DIFFUSION
= VISIBILITY + TRUTH + LEGITIMATE DESIRE + FREEDOM TO REFUSE
```

## X. Relation to Open Social Engineering

Manifesto VIII recognises that every architecture shapes conduct and requires transparency, proportionality, reversibility and control.

This manifesto defines its commercial opposite: secret behavioural engineering directed towards capturing attention, purchase or adherence through cognitive asymmetry.

Open social engineering educates while declaring its purpose. Anti-humanist neuromarketing conditions while hiding its purpose.

## XI. Neodialectical audit of campaigns

Every campaign of significant impact should be able to declare the product, cause or conduct promoted; public and foreseeable vulnerabilities; data used; persuasion techniques; AI intervention; verifiable claims; frequency and duration; rejection mechanisms; risks and mitigations; and the person responsible for review and repair.

## XII. Civilisational programme

The framework will promote literacy concerning persuasion and dark patterns, enhanced protection for minors and vulnerable people, transparency of segmentation and political advertising, limits upon emotional and biometric data, rights to pause, refuse and appeal, audits of persuasive AI, prohibition of extreme capture techniques and marketing compatible with autonomy, truth and the Common Good.

## XIII. Proclamation

> We do not condemn emotion, beauty, narrative or the ability to persuade.
>
> We condemn turning knowledge of the human mind into a secret weapon against humanity itself.
>
> We will not accept reducing a person to exploitable biases, converting loneliness into a market segment or optimising fear as conversion.
>
> We will communicate forcefully. We will show value. We will ask for support. We will compete.
>
> But we will leave the door of refusal open.
>
> A decision obtained by destroying the capacity to decide is not a choice: it is capture.

## XIV. Open Synthesis

Contributions are requested concerning persuasion, dark patterns, minors, vulnerability, biometrics, emotional data, political advertising, persuasive AI, rights of refusal and neodialectical marketing.

* [Open the Synthesis of this manifesto · Issue #25](https://github.com/PedroAlhambra/Innova-n.org-NEOCORE-NEODIALECTICA-NEODIALECTIC/issues/25)
* [Current operational contribution protocol](../../propuestas/sintesis-abierta/APORTAR_A_LA_SINTESIS_ES_EN.md)

### Equivalent internal links

* [Open Synthesis protocol](../../propuestas/sintesis-abierta/APORTAR_A_LA_SINTESIS_ES_EN.md)
* [Open Synthesis index](../../propuestas/sintesis-abierta/README.md)
* [Internal reference · 30_coherencia_fines_medios_ES_EN.md](XXX_coherencia_fines_medios_ES_EN.md)
* [Internal reference · 32_reversion_ideologica_neodialectica_mcluhan_neo0_ES_EN.md](XXXII_reversion_ideologica_neodialectica_mcluhan_neo0_ES_EN.md)
* [Manifesto index](../README.md)
* [Dedicated Open Synthesis · Issue #25](https://github.com/PedroAlhambra/Innova-n.org-NEOCORE-NEODIALECTICA-NEODIALECTIC/issues/25)

<!-- NEO_RELATIONS_START -->
## Relaciones internas y trabajo aplicado / Internal relations and applied work

- [Mapa transversal Manifiestos ↔ trabajo aplicado / Transversal Manifestos ↔ applied-work map](../RELACIONES_TRABAJO_APLICADO_ES_EN.md)

**Artículos, análisis y auditorías relacionados / Related articles, analyses and audits:**
- [Auditoría Pública Innova_N™ · Informe científico compendio sobre DistroKid, Spotify y trazabilidad de regalías](../../analisis/2025-12_Evento-Reflejo_Auditoria-Distrokid.md)
- [Actualización pública · 4 de agosto de 2026](../../analisis/2026-08-04_Actualizacion_Auditoria-DistroKid-Spotify.md)
- [LinkedIn como red profesional fragmentada](../../analisis/publicos/2026-04-15-linkedin-como-red-profesional-fragmentada.md)
- [De la economía de la atención a la economía del aporte / From the Attention Economy to the Contribution Economy](../../analisis/publicos/2026-08-05_de-la-economia-de-la-atencion-a-la-economia-del-aporte_ES_EN.md)
- [Actualización del expediente DistroKid–Spotify · trazabilidad de regalías](../../analisis/publicos/2026-08-06_actualizacion_spotify_distrokid_trazabilidad_regalias_ES_EN.md)
- [Segundo escalado a DistroKid · ausencia de respuesta y reapertura formal](../../analisis/publicos/2026-08-06_segundo_escalado_distrokid_sin_respuesta_ES_EN.md)
- [MAXPROC · Protección Integral de la Infancia™](../../analisis/publicos/2026-08-07_maxproc_proteccion_integral_infancia_punto_no_retorno_ES_EN.md)
- [Spotify–DistroKid · Cierre circular, ausencia de auditoría y nuevo escalado](../../analisis/publicos/2026-08-07_spotify_distrokid_cierre_circular_y_escalado_ES_EN.md)
- [Spotify–DistroKid · Respuesta genérica, cierre circular y nuevo escalado](../../analisis/publicos/2026-08-07_spotify_respuesta_generica_distrokid_cierre_circular_ES_EN.md)

**Manifiestos relacionados / Related manifestos:**
- XXIX · [XXIX · Manifiesto contra la Idolatría del Dinero™](XXIX_idolatria_del_dinero_ES_EN.md)
- XXVIII · [XXVIII · Manifiesto de Los Tesla™](XXVIII_los_tesla_ES_EN.md)
- XXXIV · [XXXIV · Utilidad Operativa del Marco y Auditoría Conjunta Perpetua™](XXXIV_utilidad_operativa_marco_auditoria_conjunta_perpetua_ES_EN.md)
- XXVII · [XXVII · Manifiesto del Valor de los Alimentos y la Vida™](XXVII_valor_alimentos_vida_ES_EN.md)
- XXXV · [XXXV · Contra la Ridiculez Mediática y la Economía del Conflicto™](XXXV_contra_ridiculez_mediatica_y_economia_del_conflicto_ES_EN.md)
- XXXVIII · [XXXVIII · Manifiesto de Protección Integral de la Infancia™](XXXVIII_proteccion_integral_infancia_punto_no_retorno_ES_EN.md)
- XXIII · [XXIII · Manifiesto de la Soberanía del Tiempo Cognitivo™](XXIII_soberania_tiempo_cognitivo_ES_EN.md)
- XXII · [XXII · Manifiesto contra la Reducción y la Captura Intelectual™](XXII_contra_reduccion_captura_intelectual_ES_EN.md)
- XXI · [XXI · Manifiesto por el Reconocimiento Neodialéctico™](XXI_reconocimiento_neodialectico_ES_EN.md)
- XLII · [XLII · Fin de la Era del Hombre Manipulado™ · IA, Despertar y Soberanía Cognitiva](XLII_fin_era_hombre_manipulado_ia_despertar_soberania_cognitiva_ES_EN.md)

La relación documental indica afinidad conceptual, genealogía, contraste o aplicación; no implica causalidad ni equivalencia probatoria. / Documentary relation indicates conceptual affinity, genealogy, contrast or application; it does not imply causation or evidentiary equivalence.
<!-- NEO_RELATIONS_END -->

## Navigation

← [XXX · Coherence between Ends and Means™](XXX_coherencia_fines_medios_ES_EN.md) · [Index](../README.md) · [XXXII · Neodialectical Ideological Reversal™](XXXII_reversion_ideologica_neodialectica_mcluhan_neo0_ES_EN.md) →

---

<!-- NEO_RELATED_WORK_START -->
## Trabajo aplicado y casos relacionados / Applied work and related cases

- [MAXPROC · Protección Integral de la Infancia™](../../analisis/publicos/2026-08-07_maxproc_proteccion_integral_infancia_punto_no_retorno_ES_EN.md) — menores, publicidad integrada, dark patterns, engagement y vulnerabilidad / minors, embedded advertising, dark patterns, engagement and vulnerability.
- [Matriz de evidencias de infancia](../../analisis/publicos/evidencias/2026-08-07_infancia_proteccion_consumo_educacion_medios_fuentes-y-matriz_ES_EN.md) — separa regulación, evidencia, hipótesis y afirmaciones no demostradas / separates regulation, evidence, hypotheses and unproven claims.
- [Economía de la atención → Economía del aporte](../../analisis/publicos/2026-08-05_de-la-economia-de-la-atencion-a-la-economia-del-aporte_ES_EN.md) — atención como activo y riesgo de captura / attention as asset and capture risk.
- [LinkedIn como red profesional fragmentada](../../analisis/publicos/2026-04-15-linkedin-como-red-profesional-fragmentada.md) — distribución algorítmica de visibilidad y atención / algorithmic distribution of visibility and attention.
- [Mapa transversal completo](../RELACIONES_TRABAJO_APLICADO_ES_EN.md#manifiesto-xxxi)
<!-- NEO_RELATED_WORK_END -->

<!-- NEO_OPEN_SYNTHESIS_INVITATION_START -->

## Participa en la Síntesis Abierta / Join the Open Synthesis

Puedes aportar crítica, objeciones, contraejemplos, fuentes, experiencia, verificación, implementación o delta. / You may contribute criticism, objections, counterexamples, sources, experience, verification, implementation or a delta.

**Última síntesis / Latest synthesis:** [LXVIII · Los Conflictos que No Son Nuestros™ · Soberanía Intelectual de la Especie / The Conflicts That Are Not Ours™](LXVIII_conflictos_que_no_son_nuestros_soberania_intelectual_especie_ES_EN.md) · [Issue #114](https://github.com/PedroAlhambra/Innova-n.org-NEOCORE-NEODIALECTICA-NEODIALECTIC/issues/114)  
**Seguimiento vivo / Living follow-up:** [seguir el marco / follow the framework](../../proyeccion/SEGUIR_MARCO_SINTESIS_ES_EN.md) · [registro de entrada / entry register](../../propuestas/sintesis-abierta/REGISTRO_ENTRADA_TRAZABLE_DERIVACION_ES_EN.md)  
**Auditorías / Audits:** [LIII · Leónidas™](LIII_leonidas_defensor_sintesis_auditoria_abierta_aportes_externos_ES_EN.md) · [protocolo / protocol](../../propuestas/sintesis-abierta/LEONIDAS_AUDITORIA_ABIERTA_Y_APORTES_EXTERNOS_ES_EN.md)  
**Cómo aportar / How to contribute:** [protocolo general / general protocol](../../propuestas/sintesis-abierta/APORTAR_A_LA_SINTESIS_ES_EN.md) · [portal de auditorías / audit portal](../../auditorias/publicas/README.md) · [índice / index](../../propuestas/sintesis-abierta/README.md)

<!-- NEO_OPEN_SYNTHESIS_INVITATION_END -->

<!-- NEO_RELATIONAL_FOOTER_START -->

## Relaciones y contexto / Relations and context

[Mapa transversal](../RELACIONES_TRABAJO_APLICADO_ES_EN.md) · [Síntesis relacional #98](https://github.com/PedroAlhambra/Innova-n.org-NEOCORE-NEODIALECTICA-NEODIALECTIC/issues/98) · [Mapa relacional MAXPROC](../../auditorias/publicas/2026-08-09_auditoria_relacional_manifestos_neoaxiomas_publicaciones_ES_EN.md) · [Neoaxiomas™](../../neoaxiomas/README.md) · [Índice de Síntesis Abierta](../../propuestas/sintesis-abierta/README.md)

> Este bloque añade navegación y relaciones; no sustituye, resume ni reduce el cuerpo del manifiesto. / This block adds navigation and relations; it does not replace, summarise or reduce the manifesto body.

<!-- NEO_RELATIONAL_FOOTER_END -->

<!-- NEO_MANIFESTO_NAV_START -->

## Navegación canónica / Canonical navigation

← **XXX** · [Coherencia entre Fines y Medios™ / Coherence between Ends and Means™](XXX_coherencia_fines_medios_ES_EN.md)  
· [Índice I–LXVIII / I–LXVIII index](../README.md) ·  
**XXXII** · [Reversión Ideológica Neodialéctica™ / Neodialectical Ideological Reversal™](XXXII_reversion_ideologica_neodialectica_mcluhan_neo0_ES_EN.md) →

<!-- NEO_MANIFESTO_NAV_END -->

<!-- NEO_CROSS_REFERENCES_START -->

## Referencias cruzadas canónicas / Canonical cross-references

> **Norma / Rule:** las menciones cruzadas pueden permanecer en el cuerpo como texto para no sobrecargar la lectura; este bloque concentra los hipervínculos canónicos detectados y permite retorno explícito a fuente. / Cross-references may remain as prose in the body to avoid visual overload; this block concentrates detected canonical hyperlinks and preserves explicit return to source.

- **VII** · [Manifiesto de la Economía del Aporte / Manifesto of the Contribution Economy](VII_economia_del_aporte_ES_EN.md)
- **VIII** · [Manifiesto de Ingeniería Social Abierta y Psicohistoria Neodialéctica / Manifesto of Open Social Engineering and Neodialectical Psychohistory](VIII_ingenieria_social_psicohistoria_ES_EN.md)
- **XXI** · [Manifiesto por el Reconocimiento Neodialéctico™ / Manifesto for Neodialectical Recognition™](XXI_reconocimiento_neodialectico_ES_EN.md)
- **XXII** · [Manifiesto contra la Reducción y la Captura Intelectual™ / Manifesto against Intellectual Reduction and Capture™](XXII_contra_reduccion_captura_intelectual_ES_EN.md)
- **XXIII** · [Manifiesto de la Soberanía del Tiempo Cognitivo™ / Manifesto of the Sovereignty of Cognitive Time™](XXIII_soberania_tiempo_cognitivo_ES_EN.md)
- **XXVII** · [Manifiesto del Valor de los Alimentos y la Vida™ / Manifesto of the Value of Food and Life™](XXVII_valor_alimentos_vida_ES_EN.md)
- **XXVIII** · [Manifiesto de Los Tesla™ / Manifesto of the Teslas™](XXVIII_los_tesla_ES_EN.md)
- **XXIX** · [Manifiesto contra la Idolatría del Dinero™ / Manifesto against the Idolatry of Money™](XXIX_idolatria_del_dinero_ES_EN.md)
- **XXX** · [Manifiesto de la Coherencia entre Fines y Medios™ / Manifesto of Coherence between Ends and Means™](XXX_coherencia_fines_medios_ES_EN.md)
- **XXXII** · [Manifiesto de la Reversión Ideológica Neodialéctica™ / Manifesto of Neodialectical Ideological Reversal™](XXXII_reversion_ideologica_neodialectica_mcluhan_neo0_ES_EN.md)
- **XXXIV** · [Utilidad Operativa del Marco y Auditoría Conjunta Perpetua™ / Operational Utility of the Framework and Perpetual Joint Audit™](XXXIV_utilidad_operativa_marco_auditoria_conjunta_perpetua_ES_EN.md)
- **XXXV** · [Contra la Ridiculez Mediática y la Economía del Conflicto™ / Against Media Absurdity and the Conflict Economy™](XXXV_contra_ridiculez_mediatica_y_economia_del_conflicto_ES_EN.md)
- **XXXVIII** · [Manifiesto de Protección Integral de la Infancia™ / Manifesto for the Integral Protection of Childhood™](XXXVIII_proteccion_integral_infancia_punto_no_retorno_ES_EN.md)
- **XLII** · [Fin de la Era del Hombre Manipulado™ · IA, Despertar y Soberanía Cognitiva / End of the Manipulated Human Era™ · AI, Awakening and Cognitive Sovereignty](XLII_fin_era_hombre_manipulado_ia_despertar_soberania_cognitiva_ES_EN.md)
- **LIII** · [Manifiesto Leónidas™ · Defensor de la Síntesis, la Auditoría Abierta y el Derecho a Traer Problemas / Leónidas™ Manifesto · Defender of Synthesis, Open Audit and the Right to Bring Problems](LIII_leonidas_defensor_sintesis_auditoria_abierta_aportes_externos_ES_EN.md)
- **LXVIII** · [Los Conflictos que No Son Nuestros™ · Soberanía Intelectual de la Especie / The Conflicts That Are Not Ours™ · Intellectual Sovereignty of the Species](LXVIII_conflictos_que_no_son_nuestros_soberania_intelectual_especie_ES_EN.md)

### Capas y fuentes relacionadas / Related layers and sources

- [Neoaxiomas™ / Neoaxioms™](../../neoaxiomas/README.md)
- [NEOCore™ · marco / framework](../../README.md)

<!-- NEO_CROSS_REFERENCES_END -->
